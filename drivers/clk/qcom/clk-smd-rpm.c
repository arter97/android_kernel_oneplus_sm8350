...
/* scuba */
/* SMD_XO_BUFFER */
DEFINE_CLK_SMD_RPM_XO_BUFFER(scuba, rf_clk1, rf_clk1_a, QCOM_SMD_RPM_CLK_BUF_A, 4);
DEFINE_CLK_SMD_RPM_XO_BUFFER(scuba, rf_clk2, rf_clk2_a, QCOM_SMD_RPM_CLK_BUF_A, 5);

/* Branch Voter clocks */

DEFINE_CLK_SMD_RPM_XO_BUFFER(scuba, ln_bb_clk2, ln_bb_clk2_a,
						QCOM_SMD_RPM_CLK_BUF_A, 0x2);
DEFINE_CLK_SMD_RPM_XO_BUFFER(scuba, rf_clk3, rf_clk3_a, QCOM_SMD_RPM_CLK_BUF_A, 6);
...
static int rpm_smd_clk_probe(struct platform_device *pdev)
{
	...
	int ret, i, is_holi, hw_clk_handoff = false, is_sdxnightjar, is_monaco, is_scuba;
	int is_qcs404;
	int is_bengal;
	...
	if (is_holi || is_monaco) {
		...
		/*  Hold an active set vote for the ipa_keepalive_a_clk */
		clk_prepare_enable(holi_ipa_a_clk.hw.clk);
		clk_set_rate(holi_ipa_a_clk.hw.clk, 233000000);

		/*  Hold an active set vote for the qup_keepalive_a_clk */
		clk_prepare_enable(holi_qup_a_clk.hw.clk);
		clk_set_rate(holi_qup_a_clk.hw.clk, 200000000);
	}
	...
	if (is_sdxnightjar) {
		...
		/*  Hold an active set vote for the ipa_keepalive_a_clk */
		clk_prepare_enable(bengal_ipa_a_clk.hw.clk);
		clk_set_rate(holi_ipa_a_clk.hw.clk, 233000000);

		/*  Hold an active set vote for the qup_keepalive_a_clk */
		clk_prepare_enable(bengal_qup_a_clk.hw.clk);
		clk_set_rate(holi_qup_a_clk.hw.clk, 200000000);
	}
