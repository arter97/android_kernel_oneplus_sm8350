...
static struct clk_vdd_class *gcc_direwolf_regulators[] = {
	&vdd_cx,
};
...
static struct clk_regmap *gcc_direwolf_clocks[] = {
	...
	[GCC_AGGRE_USB_NOC_NORTH_AXI_CLK] =
		&gcc_aggre_usb_noc_north_axi_clk.clkr,
	[GCC_AGGRE_USB_NOC_SOUTH_AXI_CLK] =
		&gcc_aggre_usb_noc_south_axi_clk.clkr,
	...
	[GCC_USB34_PRIM_PHY_PIPE_CLK_SRC] =
		&gcc_usb34_prim_phy_pipe_clk_src.clkr,
	...
	[GCC_USB4_1_PHY_PCIE_PIPE_CLK_SRC] =
		&gcc_usb4_1_phy_pcie_pipe_clk_src.clkr,
	...
	[GCC_USB4_PHY_P2RR2P_PIPE_CLK_SRC] =
		&gcc_usb4_phy_p2rr2p_pipe_clk_src.clkr,
...
static const struct qcom_reset_map gcc_direwolf_resets[] = {
...
static const struct regmap_config gcc_direwolf_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static const struct qcom_cc_desc gcc_direwolf_desc = {
	.config = &gcc_direwolf_regmap_config,
	.clks = gcc_direwolf_clocks,
	.num_clks = ARRAY_SIZE(gcc_direwolf_clocks),
	.resets = gcc_direwolf_resets,
	.num_resets = ARRAY_SIZE(gcc_direwolf_resets),
	.clk_regulators = gcc_direwolf_regulators,
	.num_clk_regulators = ARRAY_SIZE(gcc_direwolf_regulators),
};

static const struct of_device_id gcc_direwolf_match_table[] = {
	{ .compatible = "qcom,direwolf-gcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, gcc_direwolf_match_table);

static int gcc_direwolf_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &gcc_direwolf_desc);
	if (IS_ERR(regmap))
	...
	ret = qcom_cc_really_probe(pdev, &gcc_direwolf_desc, regmap);
	if (ret) {
...
static void gcc_direwolf_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &gcc_direwolf_desc);
}

static struct platform_driver gcc_direwolf_driver = {
	.probe = gcc_direwolf_probe,
	.driver = {
		.name = "gcc-direwolf",
		.of_match_table = gcc_direwolf_match_table,
		.sync_state = gcc_direwolf_sync_state,
	},
};

static int __init gcc_direwolf_init(void)
{
	return platform_driver_register(&gcc_direwolf_driver);
}
subsys_initcall(gcc_direwolf_init);

static void __exit gcc_direwolf_exit(void)
{
	platform_driver_unregister(&gcc_direwolf_driver);
}
module_exit(gcc_direwolf_exit);

MODULE_DESCRIPTION("QTI GCC DIREWOLF Driver");
