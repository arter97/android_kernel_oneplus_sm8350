...
#include <dt-bindings/clock/qcom,videocc-direwolf.h>
...
static struct clk_vdd_class *video_cc_direwolf_regulators[] = {
	&vdd_mm,
	&vdd_mx,
};
...
static struct clk_regmap *video_cc_direwolf_clocks[] = {
...
static const struct qcom_reset_map video_cc_direwolf_resets[] = {
...
static const struct regmap_config video_cc_direwolf_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
	.max_register = 0x1008,
...
static struct qcom_cc_desc video_cc_direwolf_desc = {
	.config = &video_cc_direwolf_regmap_config,
	.clks = video_cc_direwolf_clocks,
	.num_clks = ARRAY_SIZE(video_cc_direwolf_clocks),
	.resets = video_cc_direwolf_resets,
	.num_resets = ARRAY_SIZE(video_cc_direwolf_resets),
	.clk_regulators = video_cc_direwolf_regulators,
	.num_clk_regulators = ARRAY_SIZE(video_cc_direwolf_regulators),
};
...
static int video_cc_direwolf_probe(struct platform_device *pdev)
{
	...
	regmap = qcom_cc_map(pdev, &video_cc_direwolf_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &video_cc_direwolf_desc);
	if (ret)
		return ret;
	...
	clk_lucid_5lpe_pll_configure(&video_pll0, regmap, video_pll0.config);
	clk_lucid_5lpe_pll_configure(&video_pll1, regmap, video_pll1.config);

	ret = qcom_cc_really_probe(pdev, &video_cc_direwolf_desc, regmap);
	if (ret) {
...
static void video_cc_direwolf_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &video_cc_direwolf_desc);
}

static const struct dev_pm_ops video_cc_direwolf_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver video_cc_direwolf_driver = {
	.probe = video_cc_direwolf_probe,
	.driver = {
		.name = "video_cc-direwolf",
		.of_match_table = video_cc_direwolf_match_table,
		.sync_state = video_cc_direwolf_sync_state,
		.pm = &video_cc_direwolf_pm_ops,
	},
};

static int __init video_cc_direwolf_init(void)
{
	return platform_driver_register(&video_cc_direwolf_driver);
}
subsys_initcall(video_cc_direwolf_init);

static void __exit video_cc_direwolf_exit(void)
{
	platform_driver_unregister(&video_cc_direwolf_driver);
}
module_exit(video_cc_direwolf_exit);

MODULE_DESCRIPTION("QTI VIDEO_CC DIREWOLF Driver");
