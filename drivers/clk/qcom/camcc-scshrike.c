...
static struct clk_regmap *cam_cc_scshrike_clocks[] = {
...
static const struct regmap_config cam_cc_scshrike_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
	.max_register = 0xf0d4,
...
static const struct of_device_id cam_cc_scshrike_match_table[] = {
	{ .compatible = "qcom,scshrike-camcc" },
	{ .compatible = "qcom,scshrike-camcc-v2" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_scshrike_match_table);

static void cam_cc_scshrike_fixup_scshrikev2(struct regmap *regmap)
{
	cam_cc_pll0.config->test_ctl_hi_val = 0x00000000;
	cam_cc_pll1.config->test_ctl_hi_val = 0x00000000;
	cam_cc_pll2.config->config_ctl_val = 0x10000807;
	cam_cc_pll3.config->test_ctl_hi_val = 0x00000000;
	cam_cc_pll4.config->test_ctl_hi_val = 0x00000000;
	cam_cc_pll5.config->test_ctl_hi_val = 0x00000000;
	cam_cc_pll6.config->test_ctl_hi_val = 0x00000000;
	cam_cc_bps_clk_src.clkr.vdd_data.rate_max[VDD_HIGH] = 760000000;
}

static int cam_cc_scshrike_fixup(struct platform_device *pdev,
	struct regmap *regmap)
{
	...
	if (!strcmp(compat, "qcom,scshrike-v2-camcc"))
		cam_cc_scshrike_fixup_scshrikev2(regmap);

	return 0;
}

static int cam_cc_scshrike_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;
	...
	regmap = qcom_cc_map(pdev, &cam_cc_scshrike_desc);
	if (IS_ERR(regmap)) {
	...
	ret = cam_cc_scshrike_fixup(pdev, regmap);
	if (ret)
		goto destroy_pm_clk;
	...
	ret = qcom_cc_really_probe(pdev, &cam_cc_scshrike_desc, regmap);
	if (ret) {
...
static struct platform_driver cam_cc_scshrike_driver = {
	.probe = cam_cc_scshrike_probe,
	.driver = {
...
static int __init cam_cc_scshrike_init(void)
{
...
{
	platform_driver_unregister(&cam_cc_scshrike_driver);
}
module_exit(cam_cc_scshrike_exit);

MODULE_DESCRIPTION("QTI CAM_CC SCSHRIKE Driver");
