...
#include <dt-bindings/clock/qcom,camcc-direwolf.h>
...
static struct clk_vdd_class *cam_cc_direwolf_regulators[] = {
	&vdd_mm,
	&vdd_mx,
};
...
static struct clk_regmap *cam_cc_direwolf_clocks[] = {
...
static const struct qcom_reset_map cam_cc_direwolf_resets[] = {
...
static const struct regmap_config cam_cc_direwolf_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static struct qcom_cc_desc cam_cc_direwolf_desc = {
	.config = &cam_cc_direwolf_regmap_config,
	.clks = cam_cc_direwolf_clocks,
	.num_clks = ARRAY_SIZE(cam_cc_direwolf_clocks),
	.resets = cam_cc_direwolf_resets,
	.num_resets = ARRAY_SIZE(cam_cc_direwolf_resets),
	.clk_regulators = cam_cc_direwolf_regulators,
	.num_clk_regulators = ARRAY_SIZE(cam_cc_direwolf_regulators),
};

static const struct of_device_id cam_cc_direwolf_match_table[] = {
	{ .compatible = "qcom,direwolf-camcc" },
	{ }
};
MODULE_DEVICE_TABLE(of, cam_cc_direwolf_match_table);

static int cam_cc_direwolf_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	int ret;

	regmap = qcom_cc_map(pdev, &cam_cc_direwolf_desc);
	if (IS_ERR(regmap))
		return PTR_ERR(regmap);

	ret = qcom_cc_runtime_init(pdev, &cam_cc_direwolf_desc);
	if (ret)
		return ret;
	...
	clk_lucid_5lpe_pll_configure(&cam_cc_pll0, regmap, cam_cc_pll0.config);
	clk_lucid_5lpe_pll_configure(&cam_cc_pll1, regmap, cam_cc_pll1.config);
	clk_zonda_5lpe_pll_configure(&cam_cc_pll2, regmap, cam_cc_pll2.config);
	clk_lucid_5lpe_pll_configure(&cam_cc_pll3, regmap, cam_cc_pll3.config);
	clk_lucid_5lpe_pll_configure(&cam_cc_pll4, regmap, cam_cc_pll4.config);
	clk_lucid_5lpe_pll_configure(&cam_cc_pll5, regmap, cam_cc_pll5.config);
	clk_lucid_5lpe_pll_configure(&cam_cc_pll6, regmap, cam_cc_pll6.config);
	clk_lucid_5lpe_pll_configure(&cam_cc_pll7, regmap, cam_cc_pll7.config);

	ret = qcom_cc_really_probe(pdev, &cam_cc_direwolf_desc, regmap);
	if (ret) {
...
static void cam_cc_direwolf_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &cam_cc_direwolf_desc);
}

static const struct dev_pm_ops cam_cc_direwolf_pm_ops = {
	SET_RUNTIME_PM_OPS(qcom_cc_runtime_suspend, qcom_cc_runtime_resume, NULL)
	SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
				pm_runtime_force_resume)
};

static struct platform_driver cam_cc_direwolf_driver = {
	.probe = cam_cc_direwolf_probe,
	.driver = {
		.name = "cam_cc-direwolf",
		.of_match_table = cam_cc_direwolf_match_table,
		.sync_state = cam_cc_direwolf_sync_state,
		.pm = &cam_cc_direwolf_pm_ops,
	},
};

static int __init cam_cc_direwolf_init(void)
{
	return platform_driver_register(&cam_cc_direwolf_driver);
}
subsys_initcall(cam_cc_direwolf_init);

static void __exit cam_cc_direwolf_exit(void)
{
	platform_driver_unregister(&cam_cc_direwolf_driver);
}
module_exit(cam_cc_direwolf_exit);

MODULE_DESCRIPTION("QTI CAM_CC DIREWOLF Driver");
