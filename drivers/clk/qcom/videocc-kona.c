...
static void video_cc_kona_fixup_konav2(struct regmap *regmap)
{
	...
	video_cc_mvs1_clk_src.clkr.vdd_data.rate_max[VDD_LOWER] = 840000000;
	video_cc_mvs1_clk_src.clkr.vdd_data.rate_max[VDD_LOW] = 1098000000;
}
...
static void video_cc_kona_sync_state(struct device *dev)
{
	qcom_cc_sync_state(dev, &video_cc_kona_desc);
}

static const struct dev_pm_ops video_cc_kona_pm_ops = {
...
static struct platform_driver video_cc_kona_driver = {
	...
	.driver	= {
		...
		.sync_state = video_cc_kona_sync_state,
		.pm = &video_cc_kona_pm_ops,
	},
};
...
MODULE_ALIAS("watform:video_cc-kona");
