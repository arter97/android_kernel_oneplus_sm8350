...
void iris_send_timing_switch_pkt(void)
{
	...
	if ((refresh_rate == HIGH_FREQ) && (v_active == FHD_H || v_active == FHD_H_2412))
...
static uint32_t _iris_switch_case(const u32 refresh_rate, const u32 frame_height)
{
	...
	if ((frame_height == QHD_H) || (frame_height == QHD_H_3216)) {
		pcfg->cmd_list_index = IRIS_DTSI0_PIP_IDX;
		if (cur_pt_mode)
			switch_mode = SWITCH_PT_TO_ABYP;
	}

	if (frame_height == FHD_H || frame_height == FHD_H_2412) {
		pcfg->cmd_list_index = IRIS_DTSI1_PIP_IDX;
		if (cur_pt_mode) {
			if ((pcfg->cur_v_active == QHD_H) || (pcfg->cur_v_active == QHD_H_3216))
				switch_mode = SWITCH_PT_TO_ABYP;
			else
