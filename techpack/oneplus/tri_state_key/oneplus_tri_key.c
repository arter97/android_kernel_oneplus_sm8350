			...
			short calib_upHall_MdV, short calib_upHall_DnV)
{
	...
	up_tolerance = (short)(abs(calib_UpValueMin - calib_MdValueMin) * 11 / 10);
	mid_down_tol = (short)(abs(calib_MdValueMin - calib_DnValueMin) * 4 / 10);
	down_tolerance = (short)(abs(calib_MdValueMin - calib_DnValueMin) * 11 / 10);
	up_mid_distance = (short)(abs(calib_UpValueMin - calib_MdValueMin) * 2 / 10);
	mid_down_distance = (short)(abs(calib_MdValueMin - calib_DnValueMin) * 2 / 10);
	}
...
static void init_tri_key_work(struct work_struct *work)
{
	struct extcon_dev_data *chip = container_of(work,
			struct extcon_dev_data, init_work);
	int err = 0;

	chip= g_the_chip;
	//get data when reboot
	err = oneplus_get_data(chip);
	if (err < 0) {
		TRI_KEY_LOG("tri_key:get hall data failed!\n");
		goto fail;
	}
...
//report key value
	...
	TRI_KEY_LOG("%s init successful\n", __func__);
	return;

fail:
	TRI_KEY_LOG("%s init fail\n", __func__);
}
...
int oneplus_register_hall(const char *name, struct dhall_operations *ops)
{
	...
	if (hall_count > 1) {
		INIT_WORK(&g_the_chip->init_work, init_tri_key_work);
		schedule_work(&g_the_chip->init_work);
	}
	return 0;
}
...
static int tri_key_platform_remove(struct platform_device *pdev)
{
	if (g_the_chip) {
		...
		devm_extcon_dev_unregister(g_the_chip->dev, g_the_chip->edev);
		kfree(g_the_chip);
		g_the_chip = NULL;
	}
	return 0;
...
struct platform_driver tri_key_platform_driver = {
