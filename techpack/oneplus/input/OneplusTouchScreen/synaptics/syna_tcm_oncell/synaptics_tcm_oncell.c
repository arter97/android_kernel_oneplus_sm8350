...
int syna_set_input_reporting(struct syna_tcm_data *tcm_info, bool suspend)
{
	...
	if (unlikely(tcm_info->id_info.mode != MODE_APPLICATION || tcm_info->app_status != APP_STATUS_OK)) {
...
static int syna_tcm_read_message(struct syna_tcm_data *tcm_info, unsigned char *in_buf, unsigned int length)
{
	...
	if (unlikely(header->marker != MESSAGE_MARKER)) {
	...
	if (tcm_info->report_code <= STATUS_ERROR || tcm_info->report_code == STATUS_INVALID) {
		...
		default:
			...
			if (unlikely(tcm_info->report_code != STATUS_ERROR)) {
				UNLOCK_BUFFER(tcm_info->in);
				retval = -EIO;
				goto exit;
			}
 ...
 check_padding:
	if (unlikely(tcm_info->in.buf[total_length - 1] != MESSAGE_PADDING)) {
 ...
 exit:
	if (unlikely((retval < 0) && (atomic_read(&tcm_info->command_status) == CMD_BUSY))) {
		atomic_set(&tcm_info->command_status, CMD_ERROR);
		complete(&response_complete);
	}
				  ...
				  unsigned int length, unsigned char **resp_buf, unsigned int *resp_buf_size, unsigned int *resp_length, unsigned int timeout)
{
	...
	if (unlikely(retval == 0)) {
		...
		if (unlikely(command_status != CMD_IDLE || tcm_info->report_code == STATUS_ERROR)) {
...
static int syna_tcm_get_app_info(struct syna_tcm_data *tcm_info)
{
 ...
 get_app_info:
	...
	if (unlikely(tcm_info->app_status == APP_STATUS_BOOTING || tcm_info->app_status == APP_STATUS_UPDATING)) {
		if (timeout > 0) {
...
static int syna_tcm_get_dynamic_config(struct syna_tcm_data *tcm_info, enum dynamic_config_id id, unsigned short *value)
{
	...
	if (unlikely(retval < 0 || resp_length < 2)) {
		retval = -EINVAL;
...
static int syna_set_large_thd(void *chip_data, int large_thd)
{
	...
	if (unlikely(!tcm_info || *tcm_info->in_suspend)) {
		return -1;
	}
...
static void syna_tp_shutdown(struct i2c_client *client)
{
	...
	if (unlikely(!ts->ts_ops->power_control)) {
		ret = -EINVAL;
