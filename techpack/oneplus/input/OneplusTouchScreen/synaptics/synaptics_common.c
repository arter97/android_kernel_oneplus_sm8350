...
{
	return (unsigned int)ptr[0] + (unsigned int)ptr[1] * 0x100 + (unsigned int)ptr[2] * 0x10000 + (unsigned int)ptr[3] * 0x1000000;
}
...
int synaptics_parse_header_v2(struct image_info *image_info, const unsigned char *fw_image)
{
	...
	for (i = 0; i < number_of_areas; i++) {
		...
		} else if (0 == strncmp((char *)descriptor->id_string, APP_CONFIG_ID, strlen(APP_CONFIG_ID))) {
			if (checksum != (crc32(~0, content, length) ^ ~0)) {
		...
		} else if (0 == strncmp((char *)descriptor->id_string, DISP_CONFIG_ID, strlen(DISP_CONFIG_ID))) {
			if (checksum != (crc32(~0, content, length) ^ ~0)) {
...
void synaptics_print_limit_v3(struct seq_file *s, struct touchpanel_data *ts, const struct firmware *fw)
{
	...
	for (m = 0; m < item_cnt; m++) {
		...
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			...
			} else if ((item_head->item_bit == TYPE_HYBRIDRAW_CAP) || (item_head->item_bit == TYPE_HYBRIDABS_DIFF_CBC)
...
/*
static inline bool syna_retry_estale(const long error, const unsigned int flags)
{
	return error == -ESTALE && !(flags & LOOKUP_REVAL);
}

long syna_mkdir(const char __user *pathname, umode_t mode)
{
...
retry:
	...
	if (syna_retry_estale(error, lookup_flags)) {
...
static int tp_auto_test_read_func(struct seq_file *s, void *v)
{
	...
	syna_testdata.fd = ts->result_data;
	syna_testdata.length = ts->result_max_len;
	syna_testdata.pos = &ts->result_cur_len;
