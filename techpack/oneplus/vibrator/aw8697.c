...
static void aw8697_ram_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		pr_err("%s: failed to read %s\n", __func__,
		       aw8697_ram_name[aw8697->ram_bin_index]);
		release_firmware(cont);
#ifdef AWINIC_READ_BIN_FLEXBALLY
		if (load_count <= 20) {
	...
	pr_info("%s: loaded %s - size: %zu\n", __func__,
		aw8697_ram_name[aw8697->ram_bin_index], cont ? cont->size : 0);
	/* check sum */
...
static int aw8697_haptic_start(struct aw8697 *aw8697)
{
	...
	aw8697->game_microsecond =
		(aw8697->t_start.tv_sec - aw8697->t_stop.tv_sec) * 1000000 +
...
{
	aw8697_i2c_write(aw8697, AW8697_REG_DATDBG,
			 aw8697_haptic_set_level(aw8697, gain));
	return 0;
}
...
static int aw8697_haptic_rtp_init(struct aw8697 *aw8697)
{
	...
	while ((!aw8697_haptic_rtp_get_fifo_afi(aw8697)) &&
		...
		if ((aw8697->rtp_cnt <
		     aw8697->ram.base_addr)) { //first frame rtp_cnt is 0
			if ((aw8697_rtp->len - aw8697->rtp_cnt) <
			    (aw8697->ram.base_addr)) {
				buf_len =
					aw8697_rtp->len -
					aw8697->rtp_cnt; //rtp len less than fifo space,buf len is rtp cnt
			} else {
				buf_len =
					(aw8697->ram
						 .base_addr); //first frame buf len is fifo space
			}
		...
		if (aw8697->rtp_cnt == aw8697_rtp->len || !buf_len ||
		    (!(reg_val & 0x0f) &&
		     (aw8697->rtp_cnt >= RTP_INIT_OVERSTEP_THR))) {
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
	...
	if (aw8697->sin_add_flag == 1) {
		...
		aw8697_rtp = vmalloc(rtp_file->size + aw8697->sin_data_lenght +
				     sizeof(int));
		memset(aw8697_rtp, 0,
		       (rtp_file->size + aw8697->sin_data_lenght +
			sizeof(int)));
 ...
 *****************************************************/
static int
aw8697_haptic_audio_tp_list_match(struct shake_point *pt_info,
				  struct haptic_audio_trust_zone *p_tmp)
{
	if ((pt_info->x + AW8697_HAPTIC_AI_X_JITTER * p_tmp->w /
				  AW8697_HAPTIC_AI_X_DFT_W >
	     p_tmp->x) &&
	    (pt_info->x < p_tmp->x + p_tmp->w +
				  AW8697_HAPTIC_AI_X_JITTER * p_tmp->w /
					  AW8697_HAPTIC_AI_X_DFT_W) &&
	    (pt_info->y + AW8697_HAPTIC_AI_Y_JITTER * p_tmp->h /
				  AW8697_HAPTIC_AI_Y_DFT_H >
	     p_tmp->y) &&
	    (pt_info->y < p_tmp->y + p_tmp->h +
				  AW8697_HAPTIC_AI_Y_JITTER * p_tmp->h /
					  AW8697_HAPTIC_AI_Y_DFT_H)) {
...
static int
aw8697_haptic_audio_tz_list_match(struct haptic_audio_trust_zone *p_tmp,
				  struct trust_zone_info *p_tmp_new)
{
	...
	if ((ABS(deta_x) <
	     AW8697_HAPTIC_AI_X_JITTER * w * 2 / AW8697_HAPTIC_AI_X_DFT_W) &&
	    (ABS(deta_y) <
	     AW8697_HAPTIC_AI_X_JITTER * h * 2 / AW8697_HAPTIC_AI_Y_DFT_H) &&
	    (ABS(deta_y_h) <
	     AW8697_HAPTIC_AI_X_JITTER * h * 2 / AW8697_HAPTIC_AI_Y_DFT_H) &&
	    (ABS(deta_x_w) <
	     AW8697_HAPTIC_AI_X_JITTER * w * 2 / AW8697_HAPTIC_AI_X_DFT_W)) {
	...
	pr_debug(
		"match = %d,(%d,%d)(%d,%d),(%d,%d)(%d,%d),deta_x = %d,deta_y = %d,deta_x_w = %d,deta_y_h = %d\n",
		match, p_tmp->x, p_tmp->y, p_tmp->x + p_tmp->w,
		p_tmp->y + p_tmp->h, p_tmp_new->x, p_tmp_new->y,
		p_tmp_new->x + p_tmp_new->w, p_tmp_new->y + p_tmp_new->h,
		deta_x, deta_y, deta_x_w, deta_y_h);
	return match;
}
...
static int
aw8697_haptic_audio_tz_score_list_clear(struct haptic_audio *haptic_audio)
{
	struct haptic_audio_trust_zone *p_tmp = NULL;
...
static int aw8697_haptic_audio_tz_list_insert(struct haptic_audio *haptic_audio,
...
static int aw8697_fb_notifier_callback_tp(struct notifier_block *self,
		...
		list_for_each_entry (p_tmp, &(haptic_audio->list), list) {
			if ((tp->id[i].pt_info.x +
				...
				pr_debug(
					...
					__func__, tp->id[i].pt_info.id,
					tp->id[i].pt_info.x,
					tp->id[i].pt_info.y, p_tmp->level,
					p_tmp->x, p_tmp->y, p_tmp->w, p_tmp->h);
				if (tp->virtual_id == i) {
					tp->id[AW8697_HAPTIC_TP_ID_MAX]
						.pt_info.id =
						tp->id[i].pt_info.id;
					tp->id[AW8697_HAPTIC_TP_ID_MAX]
						.pt_info.x =
					...
					tp->id[AW8697_HAPTIC_TP_ID_MAX]
						.pt_info.status =
						AW8697_HAPTIC_TP_ST_RELEASE;
					tp->id[AW8697_HAPTIC_TP_ID_MAX].tp_flag =
						AW8697_HAPTIC_TP_RELEASE;
					tp->id[AW8697_HAPTIC_TP_ID_MAX]
						.release_flag =
						AW8697_HAPTIC_TP_RELEASE;
					do_gettimeofday(
	...
	if (AW8697_HAPTIC_CMD_TP ==
		...
		for (i = 0; i < AW8697_HAPTIC_TP_ID_MAX + 1; i++) {
			...
			if (AW8697_HAPTIC_PLAY_ENABLE ==
			    aw8697->haptic_audio.ctr.play) {
				if (AW8697_HAPTIC_TP_PRESS ==
				    tp->id[i].press_flag) {
					if (AW8697_HAPTIC_TP_PLAY_NULL ==
					    tp->id[i].play_flag) {
						if (press_delay >
						    tp->press_delay_max) {
							/* no play time > tp-play delay max time */
							tp->id[i].play_flag =
								AW8697_HAPTIC_TP_PLAY_NOMORE;
							tp->id[i].press_flag =
								AW8697_HAPTIC_TP_NULL;
							tp->id[i].release_flag =
								AW8697_HAPTIC_TP_NULL;
							if (i ==
							    AW8697_HAPTIC_TP_ID_MAX) {
								tp->id[i]
									.pt_info
									.status =
									AW8697_HAPTIC_TP_ST_RELEASE;
					...
					} else if (AW8697_HAPTIC_TP_PLAY_ENABLE ==
						   tp->id[i].play_flag) {
					}
					pr_debug(
				...
				tp->id[i].no_play_cnt = 0;
			} else if (AW8697_HAPTIC_PLAY_NULL ==
				   aw8697->haptic_audio.ctr.play) {
				if (AW8697_HAPTIC_TP_PRESS ==
				    tp->id[i].press_flag) {
					if (AW8697_HAPTIC_TP_PLAY_ENABLE ==
		...
		for (i = 0; i < AW8697_HAPTIC_TP_ID_MAX + 1; i++) {
			if (AW8697_HAPTIC_TP_PLAY_ENABLE ==
				...
				list_for_each_entry (
					...
					match_re =
						aw8697_haptic_audio_tp_list_match(
							&(tp->id[i].pt_info),
							p_tmp);
					if (match_re == 1) {
						tp->play_flag =
		...
		if ((tp->tp_ai_match_flag == 0) &&
			...
			if ((AW8697_HAPTIC_TP_PLAY_ENABLE ==
				...
				for (i = 0; i < AW8697_HAPTIC_TP_ID_MAX + 1;
				     i++) {
					if (tp->id[i].play_flag) {
						list_for_each_entry (
							...
							&(haptic_audio
								  ->score_list),
							list) {
							if (aw8697_haptic_audio_tp_list_match(
								    &(tp->id[i]
									      .pt_info),
								    p_tmp)) {
								p_tmp->level =
	...
	if (AW8697_HAPTIC_CMD_ENABLE ==
		...
		if (AW8697_HAPTIC_PLAY_ENABLE ==
			...
			if (AW8697_HAPTIC_CMD_TP ==
				...
				aw8697_haptic_set_gain(
					aw8697, aw8697->haptic_audio.ctr.gain);
...
{
	struct aw8697 *aw8697 =
		container_of(work, struct aw8697, haptic_ram_work);

	aw8697_haptic_play_mode(aw8697, AW8697_HAPTIC_RTP_MODE);
	aw8697_haptic_start(aw8697);
	aw8697_i2c_writes(aw8697, AW8697_REG_RTP_DATA, &aw8697->rtp_ptr[4],
			  *((uint32_t *)aw8697->rtp_ptr));
}
...
static int aw8697_haptic_init(struct aw8697 *aw8697)
{
	...
	aw8697_i2c_write_bits(aw8697, AW8697_REG_ANADBG,
			      AW8697_BIT_ANADBG_IOC_OCDT_MASK,
			      AW8697_BIT_ANADBG_IOC_OCDT_150NS);
	...
	if (aw8697->count_go) {
		aw8697_haptic_cont_vbat_mode(
			aw8697, AW8697_HAPTIC_CONT_VBAT_SW_COMP_MODE);
	} else {
		aw8697_haptic_cont_vbat_mode(
			aw8697, AW8697_HAPTIC_CONT_VBAT_HW_COMP_MODE);
	}
				     ...
				     const char *buf, size_t count)
{
	...
	if (val == 0) {
		...
		if (check_factory_mode()) {
			aw8697->rtp_file_num =
				FACTORY_MODE_AT_MODE_RTP_NUMBER; //Number for AT vibration
			aw8697->sin_add_flag = 0;
		} else {
				...
				size_t count)
{
	...
	if (val != FACTORY_MODE_EFFECT_NUMBER && val != 0 &&
	    !aw8697->ignore_sync) {
...
static ssize_t aw8697_f0_cali_data_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
...
static ssize_t aw8697_f0_cali_data_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
...
static ssize_t aw8697_ignore_sync_tore(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
...
static ssize_t aw8697_haptic_osc_data_show(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
...
static ssize_t aw8697_haptic_audio_tp_time_show(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_tp_time_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_tp_input_show(struct device *dev,
						 struct device_attribute *attr,
						 char *buf)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_tp_input_store(struct device *dev,
						  struct device_attribute *attr,
						  const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	...
	if ((AW8697_HAPTIC_TP_ST_PRESS == tp_input.status) &&
		...
		list_for_each_entry (p_tmp, &(haptic_audio->list), list) {
			if ((tp_input.x + AW8697_HAPTIC_AI_X_JITTER * p_tmp->w /
						  AW8697_HAPTIC_AI_X_DFT_W >
			     p_tmp->x) &&
			    (tp_input.x <
			     p_tmp->x + p_tmp->w +
				     AW8697_HAPTIC_AI_X_JITTER * p_tmp->w /
					     AW8697_HAPTIC_AI_X_DFT_W) &&
			    (tp_input.y + AW8697_HAPTIC_AI_Y_JITTER * p_tmp->h /
						  AW8697_HAPTIC_AI_Y_DFT_H >
			     p_tmp->y) &&
			    (tp_input.y <
			     p_tmp->y + p_tmp->h +
				...
				do_gettimeofday(&tp->id[AW8697_HAPTIC_TP_ID_MAX]
							 .t_press);
				tp->virtual_id = tp_input.id;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].pt_info.id =
					tp_input.id;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].pt_info.x =
					tp_input.x;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].pt_info.y =
					tp_input.y;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].pt_info.status =
					tp_input.status;
				tp->id[AW8697_HAPTIC_TP_ID_MAX]
					.pt_info.touch_flag =
					AW8697_HAPTIC_TP_TOUCH_VAIL;
				tp->id[AW8697_HAPTIC_TP_ID_MAX]
					.pt_info.touch_outside_tz_flag =
					AW8697_HAPTIC_TP_TOUCH_VAIL;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].tp_flag =
					AW8697_HAPTIC_TP_PRESS;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].press_flag =
					AW8697_HAPTIC_TP_PRESS;
				tp->id[AW8697_HAPTIC_TP_ID_MAX].release_flag =
					AW8697_HAPTIC_TP_NULL;
		...
		pr_debug("%s: tp_press_release: status=%d, flag=%d", __func__,
...
static ssize_t aw8697_haptic_audio_ai_input_show(struct device *dev,
						 struct device_attribute *attr,
						 char *buf)
{
#ifdef TIMED_OUTPUT
	...
	list_for_each_entry (p_tmp, &(haptic_audio->list), list) {
		...
		pr_debug(
			"%s: trust zone [%d]: level=%d, x=%d, y=%d, w=%d, h=%d\n",
...
static ssize_t aw8697_haptic_audio_ai_input_store(struct device *dev,
						  struct device_attribute *attr,
						  const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
	...
	for (i = 0; i < ai_tz->num; i++) {
		...
		if (haptic_audio->tz_init) {
			list_for_each_entry (p_tmp, &haptic_audio->list, list) {
				if (aw8697_haptic_audio_tz_list_match(
					    p_tmp, &ai_tz_info[i])) {
					match = 1;
					break;
				}
		...
		if (match != 1)
			aw8697_haptic_audio_tz_list_insert(haptic_audio,
							   &ai_tz_info[i]);
	}
...
static ssize_t aw8697_haptic_audio_tp_size_show(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_tp_size_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_tz_cnt_show(struct device *dev,
					       struct device_attribute *attr,
					       char *buf)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_tz_cnt_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
...
static ssize_t aw8697_haptic_audio_hap_cnt_max_outside_tz_show(
...
static ssize_t
aw8697_haptic_audio_hap_cnt_max_outside_tz_store(struct device *dev,
						 struct device_attribute *attr,
						 const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
...
static DEVICE_ATTR(f0_cali_data, S_IWUSR | S_IRUGO, aw8697_f0_cali_data_show,
		   aw8697_f0_cali_data_store);
...
static DEVICE_ATTR(ignore_store, S_IWUSR | S_IRUGO, aw8697_ignore_sync_show,
		   aw8697_ignore_sync_tore);
...
static DEVICE_ATTR(haptic_audio_tp_time, S_IWUSR | S_IRUGO,
		   aw8697_haptic_audio_tp_time_show,
		   aw8697_haptic_audio_tp_time_store);
static DEVICE_ATTR(haptic_audio_tp_input, S_IWUSR | S_IRUGO,
		   aw8697_haptic_audio_tp_input_show,
		   aw8697_haptic_audio_tp_input_store);
static DEVICE_ATTR(haptic_audio_ai_input, S_IWUSR | S_IRUGO,
		   aw8697_haptic_audio_ai_input_show,
		   aw8697_haptic_audio_ai_input_store);
static DEVICE_ATTR(haptic_audio_tp_size, S_IWUSR | S_IRUGO,
		   aw8697_haptic_audio_tp_size_show,
		   aw8697_haptic_audio_tp_size_store);
static DEVICE_ATTR(haptic_audio_tz_cnt, S_IWUSR | S_IRUGO,
		   aw8697_haptic_audio_tz_cnt_show,
		   aw8697_haptic_audio_tz_cnt_store);
static DEVICE_ATTR(haptic_audio_hap_cnt_max_outside_tz, S_IWUSR | S_IRUGO,
		   aw8697_haptic_audio_hap_cnt_max_outside_tz_show,
		   aw8697_haptic_audio_hap_cnt_max_outside_tz_store);
static DEVICE_ATTR(haptic_osc_data, S_IWUSR | S_IRUGO,
		   aw8697_haptic_osc_data_show, aw8697_osc_data_store);
...
#endif
	aw8697->vibrator_on =
		wakeup_source_register(aw8697->dev, "aw_stay_awake");
			   ...
			   struct device_node *np)
{
	...
	aw8697->count_go = of_property_read_bool(np, "op,count_go");
...
static int aw8697_pinctrl(struct aw8697 *aw8697)
{
	...
	aw8697->pinctrl_state_active =
		pinctrl_lookup_state(aw8697->pinctrl, "default");
	if (IS_ERR_OR_NULL(aw8697->pinctrl_state_active)) {
		dev_err(aw8697->dev, "Can not lookup default\n");
