...
extern void sort(void *base, size_t num, size_t size, int (*cmp_func)(const void *, const void *), void (*swap_func)(void *, void *, int size));
...
bool oplus_short_c_batt_is_prohibit_chg(struct oplus_chg_chip *chip)
{
	...
	if (chip->short_c_batt.is_switch_on) {
		if (chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__CV_ERR_CODE1 || chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__FULL_ERR_CODE2 ||
...
{
	if (chip->short_c_batt.batt_chging_cycle_threshold < 1 || chip->short_c_batt.batt_chging_cycles < 0 || chip->short_c_batt.cv_timer1 < 1 ||
	    chip->short_c_batt.cv_timer1 > chip->limits.max_chg_time_sec || chip->short_c_batt.full_timer2 < 1 ||
	    chip->short_c_batt.full_timer2 > chip->limits.max_chg_time_sec || chip->short_c_batt.full_timer3 < 1 ||
	    chip->short_c_batt.full_timer3 > chip->limits.max_chg_time_sec || chip->short_c_batt.full_timer4 < 1 ||
	    chip->short_c_batt.full_timer4 > chip->limits.max_chg_time_sec || chip->short_c_batt.full_timer5 < 1 ||
	    chip->short_c_batt.full_timer5 > chip->limits.max_chg_time_sec || chip->short_c_batt.cool_temp_rbatt < 1 ||
	    chip->short_c_batt.little_cool_temp_rbatt < 1 || chip->short_c_batt.normal_temp_rbatt < 1 || chip->short_c_batt.full_delta_vbatt1_mv < 1 ||
	    chip->short_c_batt.full_delta_vbatt2_mv < 1 || chip->short_c_batt.ex2_lower_ibatt_ma < 1 || chip->short_c_batt.ex2_lower_ibatt_ma > 2000 ||
	    chip->short_c_batt.ex2_low_ibatt_ma < 1 || chip->short_c_batt.ex2_low_ibatt_ma > 2000 || chip->short_c_batt.ex2_high_ibatt_ma < 1 ||
	    chip->short_c_batt.ex2_high_ibatt_ma > 2000 || chip->short_c_batt.ex2_lower_ibatt_count < 1 || chip->short_c_batt.ex2_low_ibatt_count < 1 ||
	    chip->short_c_batt.ex2_high_ibatt_count < 1 || chip->short_c_batt.dyna1_delta_dv_mv < 1 || chip->short_c_batt.dyna2_delta_dv_mv < 1) {
		return false;
	}
 ...
 */
static int oplus_short_c_batt_write_err_code(int err_code, int temp1, int temp2, int vbatt1_mv, int vbatt2_mv, int timer_counts)
{
	struct file *fp;
	mm_segment_t old_fs;
	loff_t pos;
...
static int oplus_short_c_batt_write_chg_data(int current_ma, int volt_mv, int lower_count, int low_count, int high_count)
{
	struct file *fp;
	mm_segment_t old_fs;
	loff_t pos;
...
void oplus_chg_short_c_battery_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_is_power_off_chg(chip) == true) {
			data_mounted_count++;
			if (data_mounted_count < 30 && oplus_short_c_batt_data_is_mounted() == true) {
				oplus_short_c_batt_read_file(chip);
	...
	if ((chip->chg_ops->get_dyna_aicl_result) && (chip->chg_ops->get_dyna_aicl_result() < INPUT_LIMIT_MA_THRESHOLD)) {
	...
	if (chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__CV_ERR_CODE1 ||
	    (chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__FULL_ERR_CODE2 && chip->batt_full != true) ||
	    chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__FULL_ERR_CODE3 ||
	    (chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4 && chip->batt_full != true) ||
	    chip->short_c_batt.err_code == SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5) {
	...
	if (chip->temperature <= LITTLE_COLD_TEMP_THRESHOLD || chip->temperature > WARM_TEMP_THRESHOLD) {
		short_debug("temperature[%d]\n", chip->temperature);
		if (oplus_short_c_batt_get_cv_status(chip) == true) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX5_OUT_OF_TEMP_0_45, INVALID_CODE, chip->temperature, INVALID_CODE, INVALID_CODE);
	...
	if (chip->batt_full == true && chip->chging_on == false && chip->charging_state == CHARGING_STATUS_FULL) {
		...
		oplus_short_c_batt_write_chg_data(chip->icharging, chip->batt_volt, full_over_10ma_count, full_over_20ma_count, full_over_50ma_count);

		if (full_over_10ma_count >= chip->short_c_batt.ex2_lower_ibatt_count) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_LOWER_IBATT, INVALID_CODE, INVALID_CODE, INVALID_CODE, INVALID_CODE);
		...
		if (full_over_20ma_count >= chip->short_c_batt.ex2_low_ibatt_count) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_LOW_IBATT, INVALID_CODE, INVALID_CODE, INVALID_CODE, INVALID_CODE);
		...
		if (full_over_50ma_count >= chip->short_c_batt.ex2_high_ibatt_count) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_HIGH_IBATT, INVALID_CODE, INVALID_CODE, INVALID_CODE, INVALID_CODE);
		...
		if (full_timer5_count > chip->short_c_batt.full_timer5 && full_timer5_count < chip->short_c_batt.full_timer5 + 2) {
			...
			if (full_vbatt_2 > 0 && chip->limits.iterm_ma != 0) {
				...
				if (cv_avg_vbatt > chip->short_c_batt.short_c_bat_cv_mv + 1000 || cv_avg_vbatt < chip->short_c_batt.short_c_bat_cv_mv - 1000) {
				...
				} else if (chip->temperature <= NORMAL_TEMP_THRESHOLD) {
				...
				if (exit_check_status == true) {
					oplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_RBATT, chip->temperature, chip->temperature, full_vbatt_2, rbatt);
					chg_err("exit_check, temp [%d] full_vbatt_2[%d] rbatt[%d]\n", chip->temperature, full_vbatt_2, rbatt);
					short_debug("exit_check, rbatt[%d]\n", rbatt);
					return;
				} else {
					oplus_short_c_batt_write_exit_code(EXIT_CODE__NOT_OVER_RBATT, chip->temperature, chip->temperature, full_vbatt_2,
									   rbatt);
		...
		if (full_timer2_count > chip->short_c_batt.full_timer2 && full_timer2_count < chip->short_c_batt.full_timer2 + 2) {
		...
		if (full_timer3_count <= chip->short_c_batt.full_timer3) {
			...
			if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
				oplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER3, full_temp1, chip->temperature, INVALID_CODE,
								   INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n", chip->temperature);
		...
		if (full_timer3_count > chip->short_c_batt.full_timer3 && full_timer3_count < chip->short_c_batt.full_timer3 + 3) {
			full_vbatt_n = oplus_short_c_batt_get_average_vbatt_mv(chip, false);
			...
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				if (full_timer3_count == chip->short_c_batt.full_timer3 + 1) {
					if ((chip->short_c_batt.batt_chging_cycles < chip->short_c_batt.batt_chging_cycle_threshold) &&
						...
						oplus_short_c_batt_write_err_code(chip->short_c_batt.err_code, full_temp1, chip->temperature, full_vbatt1,
										  full_vbatt_n, full_timer3_count * 5);
					}
				}
				/* dynamic */
				if (full_timer3_count == chip->short_c_batt.full_timer3 + 2) {
					...
					if (chip->temperature <= COOL_TEMP_THRESHOLD) {
						if (chip->short_c_batt.dyna1_low_avg_dv_mv != INVALID_DATA && chip->short_c_batt.dyna1_low_avg_dv_mv >= 0) {
							if ((full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna1_low_avg_dv_mv >
							     chip->short_c_batt.dyna1_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n > chip->short_c_batt.full_delta_vbatt1_mv)) {
								short_debug("set err_code[%d]\n", SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4);
								chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4;
								oplus_short_c_batt_write_err_code(chip->short_c_batt.err_code, full_temp1, chip->temperature,
												  full_vbatt1, full_vbatt_n, INVALID_CODE);
							}
						}
					} else {
						if (chip->short_c_batt.dyna1_high_avg_dv_mv != INVALID_DATA && chip->short_c_batt.dyna1_high_avg_dv_mv >= 0) {
							if ((full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna1_high_avg_dv_mv >
							     chip->short_c_batt.dyna1_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n > chip->short_c_batt.full_delta_vbatt1_mv)) {
								short_debug("set err_code[%d]\n", SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4);
								chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4;
					...
					if (chip->short_c_batt.err_code != SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4) {
						...
						oplus_short_c_batt_write_exit_code(EXIT_CODE__TIMER3_OVERFLOW_INFO, full_temp1, chip->temperature, full_vbatt1,
										   full_vbatt_n);
					}
				}
			} else {
		...
		if (full_timer4_count <= chip->short_c_batt.full_timer4) {
			...
			oplus_short_c_batt_get_average_vbatt_mv(chip, true);
			if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
				oplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER4, full_temp1, chip->temperature, INVALID_CODE,
								   INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n", chip->temperature);
				short_debug("don't exit_check, T1[%d], T2[%d]\n", full_temp1, chip->temperature);
				goto cv_reset;
		...
		if (full_timer4_count > chip->short_c_batt.full_timer4 && full_timer4_count < chip->short_c_batt.full_timer4 + 3) {
			full_vbatt_n = oplus_short_c_batt_get_average_vbatt_mv(chip, false);
			...
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				if (full_timer4_count == chip->short_c_batt.full_timer4 + 1) {
					if ((chip->short_c_batt.batt_chging_cycles < chip->short_c_batt.batt_chging_cycle_threshold) &&
						...
						chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__FULL_ERR_CODE3;
						oplus_short_c_batt_write_err_code(chip->short_c_batt.err_code, full_temp1, chip->temperature, full_vbatt1,
										  full_vbatt_n, full_timer4_count * 5);
					}
				}
				/* dynamic */
				if (full_timer4_count == chip->short_c_batt.full_timer4 + 2) {
					...
					if (chip->temperature <= COOL_TEMP_THRESHOLD) {
						if (chip->short_c_batt.dyna2_low_avg_dv_mv != INVALID_DATA && chip->short_c_batt.dyna2_low_avg_dv_mv >= 0) {
							if ((full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna2_low_avg_dv_mv >
						...
						if (chip->short_c_batt.dyna2_high_avg_dv_mv != INVALID_DATA && chip->short_c_batt.dyna2_high_avg_dv_mv >= 0) {
							if ((full_vbatt1 - full_vbatt_n - chip->short_c_batt.dyna2_high_avg_dv_mv >
							     chip->short_c_batt.dyna2_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n > chip->short_c_batt.full_delta_vbatt2_mv)) {
								short_debug("set err_code[%d]\n", SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5);
								chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5;
					...
					if (chip->short_c_batt.err_code != SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE5) {
						...
						oplus_short_c_batt_write_exit_code(EXIT_CODE__TIMER4_OVERFLOW, full_temp1, chip->temperature, full_vbatt1,
										   full_vbatt_n);
					}
	...
	/* pre CV check */
	if (chip->batt_volt > chip->short_c_batt.short_c_bat_cv_mv && chip->icharging < 0 && (-1 * chip->icharging) < CV_IBATT_MA_THRESHOLD) {
		cv_pre_check_count++;
	...
	if (chip->short_c_batt.cv_satus == true) {
		...
		if (cv_timer1_count == chip->short_c_batt.cv_timer1) {
			...
			oplus_short_c_batt_write_err_code(SHORT_C_BATT_STATUS__CV_ERR_CODE1, cv_temp, chip->temperature, INVALID_CODE, INVALID_CODE,
							  INVALID_CODE);
