...
#define INPUT_POWER_DECREASE (4000 * 500)
...
enum oplus_chg_debug_info_notify_flag {
	...
	OPLUS_NOTIFY_CHG_SLOW_BATT_WARM_TEMP,
	...
	OPLUS_NOTIFY_CHG_SLOW_CHECK_TIME,
...
struct warp_charge_strategy *g_warp_charge_strategy_p[OPLUS_FASTCHG_OUTPUT_POWER_MAX] = { NULL, NULL, NULL, NULL };
...
struct warp_charger_type warp_charger_type_list[] = {
	{ 0x01, 5000, 4000, OPLUS_FASTCHG_OUTPUT_POWER_20W },  { 0x11, 10000, 5000, OPLUS_FASTCHG_OUTPUT_POWER_50W },
	{ 0x12, 10000, 5000, OPLUS_FASTCHG_OUTPUT_POWER_50W }, { 0x13, 5000, 4000, OPLUS_FASTCHG_OUTPUT_POWER_20W },
	{ 0x14, 10000, 6500, OPLUS_FASTCHG_OUTPUT_POWER_65W }, { 0x19, 5000, 6000, OPLUS_FASTCHG_OUTPUT_POWER_30W },
	{ 0x21, 10000, 5000, OPLUS_FASTCHG_OUTPUT_POWER_50W }, { 0x29, 5000, 6000, OPLUS_FASTCHG_OUTPUT_POWER_30W },
	{ 0x31, 10000, 5000, OPLUS_FASTCHG_OUTPUT_POWER_50W }, { 0x33, 10000, 5000, OPLUS_FASTCHG_OUTPUT_POWER_50W },
...
static void oplus_chg_send_info_dwork(struct work_struct *work)
{
	...
	mutex_lock(&oplus_chg_debug_info.dcs_info_lock);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)) || IS_BUILTIN(CONFIG_OPLUS_CHG)
	ret = kevent_send_to_user(oplus_chg_debug_info.dcs_info);
#endif
	mutex_unlock(&oplus_chg_debug_info.dcs_info_lock);

	if ((ret > 0) && (oplus_chg_debug_info.retry_cnt > 0)) {
		queue_delayed_work(oplus_chg_debug_info.oplus_chg_debug_wq, &oplus_chg_debug_info.send_info_dwork, msecs_to_jiffies(SEND_INFO_DELAY));
...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
...
#endif
	{
		...
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, OPLUS_CHG_DEBUG_EVENT_ID "$$");
#endif

		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "type@@0x%x", oplus_chg_debug_info.notify_type);

		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$flag@@0x%x", oplus_chg_debug_info.notify_flag);

		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$pre_soc@@%d", oplus_chg_debug_info.pre_soc);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$cur_soc@@%d", oplus_chg_debug_info.cur_soc);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$pre_ui_soc@@%d", oplus_chg_debug_info.pre_ui_soc);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$cur_ui_soc@@%d", oplus_chg_debug_info.cur_ui_soc);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$sleep_time@@%lu", oplus_chg_debug_info.sleep_tm_sec);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$soc_load@@%d", chip->soc_load);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$batt_fcc@@%d", chip->batt_fcc);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$charge_status@@%d", chip->prop_status);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$boot_mode@@%d", chip->boot_mode);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$chg_total_time@@%d", oplus_chg_debug_info.chg_total_time);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$chg_start_temp@@%d", oplus_chg_debug_info.chg_start_temp);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$chg_cur_temp@@%d", chip->temperature);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$chg_start_soc@@%d", oplus_chg_debug_info.chg_start_soc);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$adapter_type@@0x%x",
				(oplus_chg_debug_info.fast_chg_type << 8) | chip->real_charger_type);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$icharging@@%d", chip->icharging);

		if (oplus_chg_chg_is_normal_path(chip)) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$ibus@@%d", chip->ibus);

		} else {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$ibus@@%d", oplus_chg_debug_info.fastchg_input_current);
		}

		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$vbat@@%d", chip->batt_volt);
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "$$cool_down@@%d", chip->cool_down);
		...
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				...
				"OTHER[ %d %d %d %d %d %d ], ",
				tm.tm_year + 1900, tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, chip->charger_exist, chip->charger_type,
				chip->charger_volt, chip->prop_status, chip->boot_mode, oplus_chg_debug_info.chg_current_by_tbatt,
				oplus_chg_debug_info.chg_current_by_cooldown, chip->batt_exist, chip->batt_full, chip->chging_on, chip->in_rechging,
				chip->charging_state, oplus_chg_debug_info.total_time, chip->temperature, chip->batt_volt, chip->batt_volt_min, chip->icharging,
				chip->ibus, chip->soc, chip->ui_soc, chip->soc_load, chip->batt_rm, chip->batt_fcc, chip->vbatt_over, chip->chging_over_time,
				chip->vchg_status, chip->tbatt_status, chip->stop_voter, chip->notify_code, chip->sw_full, chip->hw_full_by_sw, chip->hw_full,
		...
		if (warp_chip) {
			ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "WARP[ %d / %d / %d / %d / %d / %d]",
					warp_chip->fastchg_allow, warp_chip->fastchg_started, warp_chip->fastchg_dummy_started, warp_chip->fastchg_to_normal,
					warp_chip->fastchg_to_warm, warp_chip->btb_temp_over);
		}
...
#ifdef OPLUS_CHG_DEBUG_TEST
		{
			...
			for (i = 0; i < OPLUS_NOTIFY_CHG_MAX_CNT; i++) {
				ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret, "%4d ", oplus_chg_debug_info.chg_cnt[i]);
			}
...
static int oplus_chg_get_warp_input_power(struct oplus_chg_chip *chip)
{
	...
	vol = oplus_chg_debug_info.warp_max_input_volt < warp_charger_type_list[i].vol ? oplus_chg_debug_info.warp_max_input_volt :
											 warp_charger_type_list[i].vol;
	cur = oplus_chg_debug_info.warp_max_input_current < warp_charger_type_list[i].cur ? oplus_chg_debug_info.warp_max_input_current :
											    warp_charger_type_list[i].cur;
...
static int oplus_chg_set_chg_flag(int index)
{
	...
	if ((oplus_chg_debug_info.fast_chg_type == FASTCHG_CHARGER_TYPE_UNKOWN) || (oplus_chg_debug_info.fast_chg_type == CHARGER_SUBTYPE_FASTCHG_WARP)) {
	...
	case OPLUS_CHG_NOTIFY_REPEAT:
		...
		if ((index == OPLUS_NOTIFY_CHG_SLOW_CHECK_TIME) && (oplus_chg_debug_info.chg_cnt[index] > policy_time) &&
		...
		for (i = oplus_chg_debug_info.warp_charge_input_current_index; i < OPLUS_CHG_WARP_INPUT_CURRENT_MAX_CNT; i++) {
			if (chip->batt_volt <= oplus_chg_debug_info.warp_charge_strategy->input_current[i].vol) {
				...
				if (oplus_chg_debug_info.warp_charge_strategy->input_current[i].time > 0) {
					if (oplus_chg_debug_info.warp_charge_cur_state_chg_time >=
					    oplus_chg_debug_info.warp_charge_strategy->input_current[i].time) {
		...
		if (warp_charge_strategy_p) {
			for (i = 0; warp_charge_strategy_p[i].capacity_range.low != -1; i++) {
				if ((warp_charge_strategy_p[i].capacity_range.low < start_soc) &&
				    (warp_charge_strategy_p[i].capacity_range.high >= start_soc) && (warp_charge_strategy_p[i].temp_range.low < start_temp) &&
					...
					chg_err("output_power_type: %d, capacity_range [%d, %d], temp_range [%d, %d]\n", output_power_type,
						warp_charge_strategy_p[i].capacity_range.low, warp_charge_strategy_p[i].capacity_range.high,
						warp_charge_strategy_p[i].temp_range.low, warp_charge_strategy_p[i].temp_range.high);
					break;
				}
			}
		}
...
static int oplus_chg_chg_is_normal_path(struct oplus_chg_chip *chip)
{
	...
	if (warp_chip->support_warp_by_normal_charger_path && (oplus_warp_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WARP))
		return 1;
...
static int oplus_chg_normal_chg_input_power_check(struct oplus_chg_chip *chip)
{
	...
	power_supply_get_property(oplus_chg_debug_info.usb_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED, &pval);
	...
	pval.intval = 0;
	power_supply_get_property(oplus_chg_debug_info.batt_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
	float_voltage = pval.intval;

	if ((chip->batt_volt * 1000) < float_voltage) {
		if (((input_current_settled * input_voltage_settled / 1000) - (chip->ibus * chip->charger_volt)) > INPUT_POWER_DECREASE) {
...
static int oplus_chg_fastchg_chg_input_power_check(struct oplus_chg_chip *chip)
{
	...
	if (((input_power + charge_power) > INPUT_POWER_DECREASE) && (!chip->led_on) && (chip->batt_volt < HIGH_POWER_FASTCHG_BATT_VOLT)) {
		oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_ADAPTER_INPUT_LOW_POWER);
...
static int oplus_chg_chg_slow_check(struct oplus_chg_chip *chip)
{
	...
	if ((oplus_chg_debug_info.pre_prop_status != chip->prop_status) && (chip->prop_status != POWER_SUPPLY_STATUS_CHARGING)) {
	...
	if (chip->charger_exist && (chip->prop_status != POWER_SUPPLY_STATUS_FULL)) {
		if ((chip->tbatt_status == BATTERY_STATUS__WARM_TEMP) || (chip->tbatt_status == BATTERY_STATUS__HIGH_TEMP)) {
			oplus_chg_set_chg_flag(OPLUS_NOTIFY_CHG_SLOW_BATT_WARM_TEMP);
		...
		if ((chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) || (chip->tbatt_status == BATTERY_STATUS__LOW_TEMP) ||
	...
	if ((oplus_chg_debug_info.pre_prop_status != chip->prop_status) && (oplus_chg_debug_info.pre_prop_status != POWER_SUPPLY_STATUS_CHARGING)) {
	...
	if (chip->warp_project) {
		{
			if ((oplus_chg_fastchg_is_allowed(chip) == 1) && (oplus_warp_get_fastchg_started() == false)) {
...
static int oplus_chg_chg_full_check(struct oplus_chg_chip *chip)
{
	...
	if ((status == POWER_SUPPLY_STATUS_FULL) && (capacity != 100) && (!(oplus_chg_debug_info.chg_full_notified_flag & (1 << 0)))) {
...
static int oplus_chg_chg_batt_aging_check(struct oplus_chg_chip *chip)
{
...
#endif
	{
		...
		{
#ifndef OPLUS_CHG_OP_DEF
			ret = power_supply_get_property(oplus_chg_debug_info.batt_psy, POWER_SUPPLY_PROP_BATTERY_FCC, &pval);
...
static int oplus_chg_chg_batt_capacity_jump_check(struct oplus_chg_chip *chip)
{
	if (oplus_chg_debug_info.cur_soc == OPLUS_CHG_BATT_INVALID_CAPACITY) {
		...
		if (oplus_chg_debug_info.soc_load_flag) {
			queue_delayed_work(oplus_chg_debug_info.oplus_chg_debug_wq, &oplus_chg_debug_info.soc_load_dwork, msecs_to_jiffies(SOC_LOAD_DELAY));
		}
...
static int oplus_chg_debug_info_parse_dt(void)
{
	...
	int size;
	u32 val[2];
	char *warp_adpater_name[] = { "warp_charge_strategy_20w", "warp_charge_strategy_30w", "warp_charge_strategy_50w", "warp_charge_strategy_65w" };
