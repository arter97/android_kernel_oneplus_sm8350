...
void oplus_usbtemp_recover_func(struct oplus_chg_chip *chip)
{
	...
	if (level == 1) {
		...
		do {
			...
			msleep(2000);
			count_time++;
		} while (!(((chip->usb_temp_r < USB_55C || chip->usb_temp_r == USB_100C) && (chip->usb_temp_l < USB_55C || chip->usb_temp_l == USB_100C)) ||
			   count_time == 30));
...
void oplus_update_usbtemp_current_status(struct oplus_chg_chip *chip)
{
	...
	if ((chip->usb_temp_l < USB_30C || chip->usb_temp_l > USB_100C) && (chip->usb_temp_r < USB_30C || chip->usb_temp_r > USB_100C)) {
...
int oplus_usbtemp_monitor_common(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (chip->tbatt_temp / 10 <= USB_50C &&
		    (((chip->usb_temp_l >= USB_57C) && (chip->usb_temp_l < USB_100C)) || ((chip->usb_temp_r >= USB_57C) && (chip->usb_temp_r < USB_100C)))) {
			pr_err("in loop 1");
...
dischg:
		...
		if (usbtemp_debug & OPEN_LOG_BIT) {
			...
			chg_err("==================usbtemp_volt_l[%d], usb_temp_l[%d], usbtemp_volt_r[%d], usb_temp_r[%d]\n", chip->usbtemp_volt_l,
				chip->usb_temp_l, chip->usbtemp_volt_r, chip->usb_temp_r);
		}
	}
	return 0;
}
