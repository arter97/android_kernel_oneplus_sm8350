...
int read_chg_strategy_data_from_node(struct device_node *node, const char *prop_str, struct oplus_chg_strategy_data *ranges)
{
...
int oplus_chg_get_chg_strategy_data_len(struct oplus_chg_strategy_data data[], int max_len)
{
	...
	for (i = 0; i < max_len; i++) {
		if (data[i].cool_temp == 0 && data[i].heat_temp == 0 && data[i].curr_data == 0 && data[i].heat_next_index == 0 && data[i].cool_next_index == 0)
			return i;
	}
	...
	while (temp_diff != 0) {
		if (temp_diff > 0 && temp > strategy->table_data[strategy->temp_region.index].heat_temp) {
			...
			if (temp > strategy->table_data[strategy->temp_region.index].heat_temp &&
				...
				strategy->temp_region.temp = strategy->table_data[strategy->temp_region.index].heat_temp;
			else
				strategy->temp_region.temp = temp;
