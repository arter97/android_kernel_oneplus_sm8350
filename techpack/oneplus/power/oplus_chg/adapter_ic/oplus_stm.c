...
static bool warp_uart_read_addr_line_and_check(unsigned int addr)
{
	...
	if (!(addr % 0x20)) {
		...
		if ((addr - 16) == ((adapter_stm8s_firmware_data[fw_line * 34 + 1] << 8) | (adapter_stm8s_firmware_data[fw_line * 34]))) {
			for (i = 0; i < 16; i++) {
...
static bool warp_adapter_update_handle(unsigned long tx_pin, unsigned long rx_pin)
{
	...
	rc = warp_uart_write_some_addr(&adapter_stm8s_firmware_data[0], (sizeof(adapter_stm8s_firmware_data) - 34));
	if (rc) {
