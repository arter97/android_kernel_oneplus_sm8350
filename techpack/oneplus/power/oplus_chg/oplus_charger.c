...
#define OPLUS_CHG_UPDATE_INTERVAL(slow) \
	round_jiffies_relative(msecs_to_jiffies(((slow) ? OPLUS_CHG_UPDATE_NO_CHARGE_INTERVAL_SEC : OPLUS_CHG_UPDATE_INTERVAL_SEC) * 1000))
#else
...
int oplus_usb_property_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int ret = 0;
	switch (psp) {
	case POWER_SUPPLY_PROP_OTG_SWITCH:
...
int oplus_usb_set_property(struct power_supply *psy, enum power_supply_property psp, const union power_supply_propval *val)
{
	int ret = 0;
...
int oplus_ac_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	if (chip->charger_exist) {
		if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP || suspend_charger) || (oplus_warp_get_fastchg_started() == true) ||
		...
		if ((oplus_warp_get_fastchg_started() == true) || (oplus_warp_get_fastchg_to_normal() == true) || (oplus_warp_get_fastchg_to_warm() == true) ||
		    (oplus_warp_get_fastchg_dummy_started() == true) || (oplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) ||
		    (oplus_warp_get_btb_temp_over() == true) || chip->mmi_fastchg == 0) {
			chip->ac_online = true;
		} else {
			chip->ac_online = false;
		}
	...
	case POWER_SUPPLY_PROP_FAST2NORMAL_CHG:
		if (oplus_warp_get_fastchg_to_normal() == true || oplus_warp_get_fastchg_to_warm() == true || oplus_warp_get_btb_temp_over() == true ||
...
int oplus_battery_set_property(struct power_supply *psy, enum power_supply_property psp, const union power_supply_propval *val)
{
	int ret = 0;
...
static void oplus_midas_chg_info(const char *name)
{
	midas_debug("%s: passedchg=%d, realpassedchg=%d,"
		    ...
		    name, midas_chg.cali_passed_chg, midas_chg.passed_chg, midas_chg.accu_delta, midas_chg.prev_chg_stat, midas_chg.reset_counts);
}
...
#ifndef CONFIG_OPLUS_CHG_GKI_SUPPORT
int oplus_battery_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	int ret = 0;
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (oplus_chg_show_warp_logo_ornot() == 1) {
			if (chip->new_ui_warning_support && (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP && chip->batt_full))
...
static bool oplus_chg_update_slow(struct oplus_chg_chip *chip)
{
	...
	slow = !(oplus_chg_is_wls_present(chip) || chip->charger_exist || (ibat_average >= 1000) || (chip->soc <= 15) || (chip->soc >= 90));
...
static int init_charger_proc(struct oplus_chg_chip *chip)
{
	...
	prEntry_tmp = proc_create_data("charger_factorymode_test", 0666, prEntry_da, &proc_charger_factorymode_test_ops, chip);
	if (prEntry_tmp == NULL) {
		ret = -1;
	...
	prEntry_tmp = proc_create_data("input_current_now", 0666, prEntry_da, &proc_charger_input_current_now_ops, chip);
	prEntry_tmp = proc_create_data("passedchg", 0666, prEntry_da, &proc_charger_passedchg_ops, chip);
	prEntry_tmp = proc_create_data("passedchg_reset_count", 0666, prEntry_da, &proc_charger_passedchg_reset_count_ops, chip);
	return 0;
}
...
#endif /* OPLUS_CHG_OP_DEF */
#endif
		if (swarp_is_control_by_warp != true && chip->boot_completed != false && chip->calculate_decimal_time == 0 && control_by_wireless == false &&
...
static void oplus_mmi_fastchg_in(struct oplus_chg_chip *chip)
{
	...
	schedule_delayed_work(&chip->mmi_adapter_in_work, round_jiffies_relative(msecs_to_jiffies(2000)));
}
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip = container_of(dwork, struct oplus_chg_chip, led_power_on_report_work);
	chip->led_on = true;
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip = container_of(dwork, struct oplus_chg_chip, ctrl_lcm_frequency);
	static int lcm_en_flag = LCM_EN_DEAFULT;

	if (((oplus_chg_get_charger_voltage() > CHG_VUSBIN_VOL_THR)) || (oplus_chg_is_wls_online(chip))) {
		if (chip->sw_full) {
		...
		mod_delayed_work(system_highpri_wq, &chip->ctrl_lcm_frequency, OPLUS_CHG_UPDATE_INTERVAL(oplus_chg_update_slow(chip)));
	} else {
		if (lcm_en_flag != LCM_EN_ENABLE) {
			lcm_en_flag = LCM_EN_ENABLE;
			chip->chg_ops->chg_lcm_en(true);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip = container_of(dwork, struct oplus_chg_chip, check_abnormal_voltage_work);
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chg = container_of(dwork, struct oplus_chg_chip, recovery_chg_type_work);
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
...
#ifdef OPLUS_CHG_OP_DEF
	rc = of_property_read_u32(node, "qcom,batt_current_camera_ma", &chip->limits.batt_current_camera_ma);
	if (rc) {
		chip->limits.batt_current_camera_ma = OPCHG_BATT_CURRENT_LIMIT_CAMERA_MA;
	}
	rc = of_property_read_u32(node, "qcom,fast_current_camera_level", &chip->limits.fast_current_camera_level);
	if (rc) {
...
#ifdef OPLUS_CHG_OP_DEF
	rc = of_property_read_u32(node, "qcom,batt_current_calling_ma", &chip->limits.batt_current_calling_ma);
	if (rc) {
		chip->limits.batt_current_calling_ma = OPCHG_BATT_CURRENT_LIMIT_CALLING_MA;
	}
	rc = of_property_read_u32(node, "qcom,fast_current_calling_level", &chip->limits.fast_current_calling_level);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,temp_little_cool_fastchg_current_ma", &chip->limits.temp_little_cool_fastchg_current_ma);
	if (rc < 0) {
		chip->limits.temp_little_cool_fastchg_current_ma = -EINVAL;
	}

	rc = of_property_read_u32(node, "qcom,temp_little_cool_fastchg_current_ma_high", &chip->limits.temp_little_cool_fastchg_current_ma_high);
	if (rc < 0) {
		chip->limits.temp_little_cool_fastchg_current_ma_high = chip->limits.temp_little_cool_fastchg_current_ma;
	}

	rc = of_property_read_u32(node, "qcom,temp_little_cool_fastchg_current_ma_low", &chip->limits.temp_little_cool_fastchg_current_ma_low);
	if (rc < 0) {
		chip->limits.temp_little_cool_fastchg_current_ma_low = chip->limits.temp_little_cool_fastchg_current_ma;
	}

	rc = of_property_read_u32(node, "qcom,pd_temp_little_cool_fastchg_current_ma", &chip->limits.pd_temp_little_cool_fastchg_current_ma);
	if (rc < 0) {
		chip->limits.pd_temp_little_cool_fastchg_current_ma = chip->limits.temp_little_cool_fastchg_current_ma;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_little_cool_fastchg_current_ma", &chip->limits.qc_temp_little_cool_fastchg_current_ma);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_phase1_vfloat_mv", &chip->limits.temp_normal_phase1_vfloat_mv);
	if (rc < 0) {
		chg_err(" temp_normal_phase1_vfloat_mv fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase1_fastchg_current_ma", &chip->limits.temp_normal_phase1_fastchg_current_ma);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_phase2_vfloat_mv", &chip->limits.temp_normal_phase2_vfloat_mv);
	if (rc < 0) {
		chg_err(" temp_normal_phase2_vfloat_mv fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase2_fastchg_current_ma_high", &chip->limits.temp_normal_phase2_fastchg_current_ma_high);
	if (rc < 0) {
		chg_err(" temp_normal_phase2_fastchg_current_ma_high fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase2_fastchg_current_ma_low", &chip->limits.temp_normal_phase2_fastchg_current_ma_low);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_phase3_vfloat_mv", &chip->limits.temp_normal_phase3_vfloat_mv);
	if (rc < 0) {
		chg_err(" temp_normal_phase3_vfloat_mv fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase3_fastchg_current_ma_high", &chip->limits.temp_normal_phase3_fastchg_current_ma_high);
	if (rc < 0) {
		chg_err(" temp_normal_phase3_fastchg_current_ma_high fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase3_fastchg_current_ma_low", &chip->limits.temp_normal_phase3_fastchg_current_ma_low);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_phase4_vfloat_mv", &chip->limits.temp_normal_phase4_vfloat_mv);
	if (rc < 0) {
		chg_err(" temp_normal_phase4_vfloat_mv fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase4_fastchg_current_ma_high", &chip->limits.temp_normal_phase4_fastchg_current_ma_high);
	if (rc < 0) {
		chg_err(" temp_normal_phase4_fastchg_current_ma_high fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase4_fastchg_current_ma_low", &chip->limits.temp_normal_phase4_fastchg_current_ma_low);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_phase5_vfloat_mv", &chip->limits.temp_normal_phase5_vfloat_mv);
	if (rc < 0) {
		chg_err(" temp_normal_phase5_vfloat_mv fail\n");
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase5_fastchg_current_ma", &chip->limits.temp_normal_phase5_fastchg_current_ma);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,temp_normal_phase6_vfloat_mv", &chip->limits.temp_normal_phase6_vfloat_mv);
	if (rc < 0) {
		chip->limits.temp_normal_phase6_vfloat_mv = chip->limits.temp_normal_phase5_vfloat_mv;
	}

	rc = of_property_read_u32(node, "qcom,temp_normal_phase6_fastchg_current_ma", &chip->limits.temp_normal_phase6_fastchg_current_ma);
	if (rc < 0) {
	...
	charger_xlog_printk(
		...
		chip->limits.temp_normal_phase3_fastchg_current_ma_high, chip->limits.temp_normal_phase3_fastchg_current_ma_low,
		chip->limits.normal_phase4_bat_decidegc, chip->limits.temp_normal_phase4_vfloat_mv, chip->limits.temp_normal_phase4_fastchg_current_ma_high,
		chip->limits.temp_normal_phase4_fastchg_current_ma_low, chip->limits.normal_phase5_bat_decidegc, chip->limits.temp_normal_phase5_vfloat_mv,
		chip->limits.temp_normal_phase5_fastchg_current_ma, chip->limits.normal_phase6_bat_decidegc, chip->limits.temp_normal_phase6_vfloat_mv,
	...
	rc = of_property_read_u32(node, "qcom,temp_warm_fastchg_current_ma", &chip->limits.temp_warm_fastchg_current_ma);
	if (rc < 0) {
		chip->limits.temp_warm_fastchg_current_ma = -EINVAL;
	}
	rc = of_property_read_u32(node, "qcom,pd_temp_warm_fastchg_current_ma", &chip->limits.pd_temp_warm_fastchg_current_ma);
	if (rc < 0) {
		chip->limits.pd_temp_warm_fastchg_current_ma = chip->limits.temp_warm_fastchg_current_ma;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_warm_fastchg_current_ma", &chip->limits.qc_temp_warm_fastchg_current_ma);
	if (rc < 0) {
		chip->limits.qc_temp_warm_fastchg_current_ma = chip->limits.temp_warm_fastchg_current_ma;
	}
	rc = of_property_read_u32(node, "qcom,temp_warm_fastchg_current_ma_led_on", &chip->limits.temp_warm_fastchg_current_ma_led_on);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,non_standard_fastchg_current_ma", &chip->limits.non_standard_fastchg_current_ma);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,short_c_bat_fastchg_current_ma", &chip->limits.short_c_bat_fastchg_current_ma);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,non_standard_vfloat_over_sw_limit", &chip->limits.non_standard_vfloat_over_sw_limit);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,little_cold_vfloat_over_sw_limit", &chip->limits.little_cold_vfloat_over_sw_limit);
	if (rc < 0) {
	...
	rc = of_property_read_u32(node, "qcom,little_cool_vfloat_over_sw_limit", &chip->limits.little_cool_vfloat_over_sw_limit);
	if (rc < 0) {
	...
	charger_xlog_printk(CHG_LOG_CRTI, "vbatt_power_off = %d, \
			...
			vbatt_pdqc_to_5v_thr = %d\n",
			    ...
			    chip->limits.vbatt_pdqc_to_9v_thr, chip->limits.vbatt_pdqc_to_5v_thr);
	...
	charger_xlog_printk(CHG_LOG_CRTI, "ff1_normal_fastchg_ma = %d, \
			...
			qc_input_current_charger_ma = %d\n",
			    chip->limits.ff1_normal_fastchg_ma, chip->limits.ffc2_temp_warm_decidegc, chip->limits.ffc2_temp_high_decidegc,
			    chip->limits.ffc2_normal_fastchg_ma, chip->limits.ffc2_warm_fastchg_ma, chip->limits.ffc2_exit_step_ma,
			    chip->limits.ffc1_normal_vfloat_sw_limit, chip->limits.ffc1_warm_vfloat_sw_limit, chip->limits.ffc2_normal_vfloat_sw_limit,
			    chip->limits.ffc2_warm_vfloat_sw_limit, chip->limits.ffc1_temp_normal_vfloat_mv, chip->limits.ffc2_temp_normal_vfloat_mv,
			    chip->limits.ffc_normal_vfloat_over_sw_limit, chip->limits.ffc2_temp_low_decidegc, chip->limits.ff1_exit_step_ma,
			    chip->limits.ff1_warm_exit_step_ma, chip->limits.pd_input_current_charger_ma, chip->limits.qc_input_current_charger_ma);
	...
	charger_xlog_printk(CHG_LOG_CRTI, "default_temp_little_cool_fastchg_current_ma = %d, \
			...
			default_little_cool_vfloat_over_sw_limit = %d\n",
			    chip->limits.default_temp_little_cool_fastchg_current_ma, chip->limits.default_little_cool_vfloat_sw_limit,
			    chip->limits.default_temp_little_cool_vfloat_mv, chip->limits.default_little_cool_vfloat_over_sw_limit);
	...
	chip->limits.default_temp_little_cool_fastchg_current_ma_high = chip->limits.temp_little_cool_fastchg_current_ma_high;
	chip->limits.default_temp_little_cool_fastchg_current_ma_low = chip->limits.temp_little_cool_fastchg_current_ma_low;
	...
	rc = of_property_read_u32(node, "qcom,input_current_warp_ma_normal", &chip->limits.input_current_warp_ma_normal);
	if (rc) {
		chip->limits.input_current_warp_ma_normal = 3600;
	}
	rc = of_property_read_u32(node, "qcom,input_current_warp_led_ma_high", &chip->limits.input_current_warp_led_ma_high);
	if (rc) {
		chip->limits.input_current_warp_led_ma_high = 1800;
	}
	rc = of_property_read_u32(node, "qcom,input_current_warp_led_ma_warm", &chip->limits.input_current_warp_led_ma_warm);
	if (rc) {
		chip->limits.input_current_warp_led_ma_warm = 1800;
	}
	rc = of_property_read_u32(node, "qcom,input_current_warp_led_ma_normal", &chip->limits.input_current_warp_led_ma_normal);
	if (rc) {
		chip->limits.input_current_warp_led_ma_normal = 1800;
	}
	rc = of_property_read_u32(node, "qcom,warp_temp_bat_normal_decidegc", &chip->limits.warp_normal_bat_decidegc);
	if (rc) {
		chip->limits.warp_normal_bat_decidegc = 340;
	}
	rc = of_property_read_u32(node, "qcom,input_current_warp_ma_warm", &chip->limits.input_current_warp_ma_warm);
	if (rc) {
		chip->limits.input_current_warp_ma_warm = 3000;
	}
	rc = of_property_read_u32(node, "qcom,warp_temp_bat_warm_decidegc", &chip->limits.warp_warm_bat_decidegc);
	if (rc) {
		chip->limits.warp_warm_bat_decidegc = 380;
	}
	rc = of_property_read_u32(node, "qcom,input_current_warp_ma_high", &chip->limits.input_current_warp_ma_high);
	if (rc) {
		chip->limits.input_current_warp_ma_high = 2600;
	}
	rc = of_property_read_u32(node, "qcom,warp_temp_bat_hot_decidegc", &chip->limits.warp_high_bat_decidegc);
	if (rc) {
		chip->limits.warp_high_bat_decidegc = 450;
	}
	rc = of_property_read_u32(node, "qcom,charger_current_warp_ma_normal", &chip->limits.charger_current_warp_ma_normal);
	if (rc) {
		chip->limits.charger_current_warp_ma_normal = 1000;
	}
	chip->limits.default_input_current_warp_ma_high = chip->limits.input_current_warp_ma_high;
	chip->limits.default_input_current_warp_ma_warm = chip->limits.input_current_warp_ma_warm;
	chip->limits.default_input_current_warp_ma_normal = chip->limits.input_current_warp_ma_normal;
	...
	charger_xlog_printk(CHG_LOG_CRTI, "input_current_charger_ma = %d, \
			...
			chip->smooth_switch = %d\n",
			    chip->limits.input_current_charger_ma, chip->limits.input_current_usb_ma, chip->limits.input_current_led_ma,
			    chip->limits.input_current_led_ma_normal, chip->limits.input_current_led_ma_warm, chip->limits.input_current_led_ma_high,
			    chip->limits.temp_normal_fastchg_current_ma, chip->limits.temp_normal_vfloat_mv, chip->limits.iterm_ma, chip->limits.recharge_mv,
			    chip->limits.cold_bat_decidegc, chip->limits.temp_cold_vfloat_mv, chip->limits.temp_cold_fastchg_current_ma,
			    chip->limits.little_cold_bat_decidegc, chip->limits.temp_little_cold_vfloat_mv, chip->limits.temp_little_cold_fastchg_current_ma,
			    chip->limits.cool_bat_decidegc, chip->limits.temp_cool_vfloat_mv, chip->limits.temp_cool_fastchg_current_ma_high,
			    chip->limits.temp_cool_fastchg_current_ma_low, chip->limits.little_cool_bat_decidegc, chip->limits.temp_little_cool_vfloat_mv,
			    chip->limits.temp_little_cool_fastchg_current_ma, chip->limits.normal_bat_decidegc, chip->limits.warm_bat_decidegc,
			    chip->limits.temp_warm_vfloat_mv, chip->limits.temp_warm_fastchg_current_ma, chip->limits.hot_bat_decidegc,
			    chip->limits.non_standard_vfloat_mv, chip->limits.non_standard_fastchg_current_ma, chip->limits.max_chg_time_sec,
			    chip->limits.charger_hv_thr, chip->limits.charger_lv_thr, chip->limits.vbatt_full_thr, chip->limits.vbatt_hv_thr,
			    chip->limits.vfloat_step_mv, chip->warp_project, chip->suspend_after_full, chip->external_gauge,
			    chip->limits.sw_vfloat_over_protect_enable, chip->limits.temp_little_cold_fastchg_current_ma_low,
			    chip->limits.temp_little_cold_fastchg_current_ma_high, chip->limits.charger_current_warp_ma_normal, chip->ffc_support,
			    chip->dual_ffc, chip->new_ui_warning_support, chip->smooth_switch);
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_chg_led_on_strategy[%d]: %d %d %d %d %d\n", i, chip->dynamic_config.swarp_chg_led_on_strategy_data[i].cool_temp,
		       chip->dynamic_config.swarp_chg_led_on_strategy_data[i].heat_temp, chip->dynamic_config.swarp_chg_led_on_strategy_data[i].curr_data,
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_chg_led_off_strategy[%d]: %d %d %d %d %d\n", i, chip->dynamic_config.swarp_chg_led_off_strategy_data[i].cool_temp,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data[i].heat_temp, chip->dynamic_config.swarp_chg_led_off_strategy_data[i].curr_data,
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_chg_led_off_strategy_high[%d]: %d %d %d %d %d\n", i, chip->dynamic_config.swarp_chg_led_off_strategy_data_high[i].cool_temp,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data_high[i].heat_temp,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data_high[i].curr_data,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data_high[i].heat_next_index,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data_high[i].cool_next_index);
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_general_chg_strategy_low[%d]: %d %d %d %d %d\n", i, chip->dynamic_config.swarp_general_chg_strategy_data_low[i].cool_temp,
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_general_chg_strategy_high[%d]: %d %d %d %d %d\n", i, chip->dynamic_config.swarp_general_chg_strategy_data_high[i].cool_temp,
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	if (((!chip->authenticate) || (!chip->hmac)) && (charging_current > chip->limits.non_standard_fastchg_current_ma)) {
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ctrl_by_warp && chip->vbatt_num == 2 && oplus_warp_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WARP &&
		...
		chg_err("chg_ctrl_by_warp,  \
			...
			chip->led_on, chip->calling_on, current_limit, chip->warp_temp_status);
	...
	charger_xlog_printk(CHG_LOG_CRTI, "led_on = %d, \
		...
		current_limit = %d, \
		led_temp_status = %d\n",
			    chip->led_on, chip->chg_ctrl_by_cool_down, current_limit, chip->led_temp_status);
#endif
out:
...
static void oplus_chg_vfloat_over_check(struct oplus_chg_chip *chip)
{
...
#else /* OPLUS_CHG_OP_DEF */
	if (is_comm_ocm_available(chip) && oplus_chg_comm_get_ffc_status(chip->comm_ocm) != FFC_DEFAULT) {
		return;
	}
#endif /* OPLUS_CHG_OP_DEF */
#endif
	...
	if (chip->limits.sw_vfloat_over_protect_enable) {
		if ((chip->batt_volt >= chip->limits.cold_vfloat_over_sw_limit && chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) ||
		    (chip->batt_volt >= chip->limits.little_cold_vfloat_over_sw_limit && chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) ||
		    (chip->batt_volt >= chip->limits.cool_vfloat_over_sw_limit && chip->tbatt_status == BATTERY_STATUS__COOL_TEMP) ||
		    (chip->batt_volt >= chip->limits.little_cool_vfloat_over_sw_limit && chip->tbatt_status == BATTERY_STATUS__LITTLE_COOL_TEMP) ||
		    (chip->batt_volt >= chip->limits.normal_vfloat_over_sw_limit && chip->tbatt_status == BATTERY_STATUS__NORMAL) ||
		    (chip->batt_volt >= chip->limits.warm_vfloat_over_sw_limit && chip->tbatt_status == BATTERY_STATUS__WARM_TEMP) ||
...
{
	if (chip->suspend_after_full) {
		if ((chip->tbatt_status == BATTERY_STATUS__HIGH_TEMP || chip->tbatt_status == BATTERY_STATUS__WARM_TEMP) && (chip->batt_volt < 4250)) {
			return CHG_DISABLE;
		} else {
			return CHG_SUSPEND;
		}
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	if (tbatt_status != chip->tbatt_status) {
		if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP || chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) {
			if (chip->soc != 100 && chip->batt_full == true && chip->charging_state == CHARGING_STATUS_FULL) {
...
static void oplus_chg_check_tbatt_normal_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_normal_status != tbatt_normal_status) {
		...
		chg_debug("tbatt_normal_status status change, [%d %d %d %d %d %d %d]\n", chip->tbatt_normal_status, chip->limits.normal_phase1_bat_decidegc,
			  chip->limits.normal_phase2_bat_decidegc, chip->limits.normal_phase3_bat_decidegc, chip->limits.normal_phase4_bat_decidegc,
...
static void oplus_chg_check_tled_status(struct oplus_chg_chip *chip)
{
	...
	if (tled_status != chip->led_temp_status) {
		...
		chg_debug("tled status change, [%d %d %d %d]\n", tled_status, chip->led_temp_status, chip->limits.led_warm_bat_decidegc_antishake,
...
static void oplus_chg_check_led_on_ichging(struct oplus_chg_chip *chip)
{
	if (chip->led_on_change || (chip->led_on && chip->led_temp_change)) {
		...
		if (chip->chg_ctrl_by_warp && chip->vbatt_num == 2) {
			if (oplus_warp_get_fastchg_started() == true && oplus_warp_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WARP) {
				return;
			}
			if (oplus_warp_get_allow_reading() == true || oplus_warp_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WARP) {
				oplus_chg_set_charging_current(chip);
				oplus_chg_set_input_current_limit(chip);
			}
		} else {
...
static void oplus_chg_check_warp_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->vbatt_num != 2 || oplus_warp_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WARP || oplus_warp_get_fastchg_started() == false) {
	...
	chg_err("tbat_warp_status[%d],chip->warp_temp_status[%d] ", tbat_warp_status, chip->warp_temp_status);
	if (warp_first_set_input_current_flag == false) {
		chip->limits.temp_little_cool_fastchg_current_ma = chip->limits.charger_current_warp_ma_normal;
		chip->limits.temp_normal_fastchg_current_ma = chip->limits.charger_current_warp_ma_normal;
		oplus_chg_set_charging_current(chip);
		chg_err("set charger current ctrl by warp[%d]\n", chip->limits.temp_little_cool_fastchg_current_ma);
	}
	if (tbat_warp_status != chip->warp_temp_status || warp_first_set_input_current_flag == false) {
		chip->limits.warp_warm_bat_decidegc_antishake = chip->limits.warp_warm_bat_decidegc;
		chip->limits.warp_normal_bat_decidegc_antishake = chip->limits.warp_normal_bat_decidegc;
		if (tbat_warp_status > chip->warp_temp_status && tbat_warp_status == WARP_TEMP_STATUS__WARM) {
			chip->limits.warp_normal_bat_decidegc_antishake = chip->limits.warp_normal_bat_decidegc - TWARP_HYSTERISIS_DECIDEGC;
		} else if (tbat_warp_status > chip->warp_temp_status && tbat_warp_status == WARP_TEMP_STATUS__HIGH) {
			chip->limits.warp_warm_bat_decidegc_antishake = chip->limits.warp_warm_bat_decidegc - TWARP_HYSTERISIS_DECIDEGC;
		} else if (tbat_warp_status < chip->warp_temp_status && tbat_warp_status == WARP_TEMP_STATUS__NORMAL) {
			chip->limits.warp_normal_bat_decidegc_antishake = chip->limits.warp_normal_bat_decidegc + TWARP_HYSTERISIS_DECIDEGC;
		} else if (tbat_warp_status < chip->warp_temp_status && tbat_warp_status == WARP_TEMP_STATUS__WARM) {
			chip->limits.warp_warm_bat_decidegc_antishake = chip->limits.warp_warm_bat_decidegc + TWARP_HYSTERISIS_DECIDEGC;
		}
		chg_debug("tled status change, [%d %d %d %d]\n", tbat_warp_status, chip->warp_temp_status, chip->limits.warp_warm_bat_decidegc_antishake,
...
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	...
	chip->notify_code = 0;
	chip->notify_flag = 0;
	if (!((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) || (oplus_warp_get_fastchg_started() == true) || (oplus_warp_get_fastchg_to_warm() == true) ||
	      (oplus_warp_get_fastchg_dummy_started() == true) || (oplus_warp_get_adapter_update_status() == ADAPTER_FW_NEED_UPDATE) ||
	      (oplus_warp_get_btb_temp_over() == true))) {
		chip->cool_down = 0;
		chip->cool_down_done = false;
	...
	chip->limits.temp_little_cool_fastchg_current_ma_high = chip->limits.default_temp_little_cool_fastchg_current_ma_high;
	chip->limits.temp_little_cool_fastchg_current_ma_low = chip->limits.default_temp_little_cool_fastchg_current_ma_low;
...
void oplus_charger_detect_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
...
#ifdef OPLUS_CHG_OP_DEF
			if (is_usb_ocm_available(chip)) {
				oplus_chg_mod_event(chip->usb_ocm, chip->usb_ocm, OPLUS_CHG_EVENT_ONLINE);
			}
#endif
			...
			} else if (oplus_warp_get_fastchg_started() == false && charger_resumed == false) {
				charger_resumed = chip->chg_ops->check_charger_resume();
				oplus_chg_turn_on_charging(chip);
			}
#ifdef OPLUS_CHG_OP_DEF
...
#endif
#ifdef OPLUS_CHG_OP_DEF
		if (is_usb_ocm_available(chip) && !oplus_warp_get_fastchg_started() && !ac_offline) {
			ac_offline = true;
			oplus_chg_mod_event(chip->usb_ocm, chip->usb_ocm, OPLUS_CHG_EVENT_OFFLINE);
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (ui_soc_cp_flag == 0) {
		if ((chip->soc < 0 || chip->soc > 100 || (chip->chg_ops->get_rtc_soc() == -ENODEV)) && retry_counts < RETRY_COUNTS) {
			charger_xlog_printk(CHG_LOG_CRTI, "[Battery]oplus_chg_get_battery_data,\
...
static void oplus_chg_check_aicl_input_limit(struct oplus_chg_chip *chip)
{
	...
	if (chip->charging_state == CHARGING_STATUS_FAIL || chip->batt_full == true ||
	    ((chip->tbatt_status != BATTERY_STATUS__NORMAL) && (chip->tbatt_status != BATTERY_STATUS__LITTLE_COOL_TEMP)) || chip->ui_soc > 85 ||
	    oplus_warp_get_fastchg_started() == true) {
		aicl_delay_count = 0;
		return;
	}
	...
	if (chip->charging_state == CHARGING_STATUS_FAIL || chip->batt_full == true ||
	    ...
	    ((chip->ui_soc > 85) && (chip->pmic_spmi.aicl_suspend == false)) || oplus_warp_get_fastchg_started() == true) {
		aicl_delay_count = 0;
		return;
	}
	if (aicl_delay_count > AICL_DELAY_15MIN) {
	...
	if (oplus_warp_get_allow_reading() == true && chip->pmic_spmi.usb_hc_mode && !chip->pmic_spmi.hc_mode_flag) {
		oplus_chg_set_input_current_limit(chip);
...
static void oplus_chg_check_chg_strategy_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->led_on) {
#ifdef CONFIG_OPLUS_CHG_OOS
		if (chip->vbatt_num == 2 && oplus_warp_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WARP && oplus_warp_get_fastchg_started() == true) {
...
void oplus_check_ovp_status(struct oplus_chg_chip *chg)
{
	...
	if ((volt_diff > VOLT_LOW_DIFF_VALUE) && (volt_diff < VOLT_HIGH_DIFF_VALUE) &&
	    ((chg->charger_volt >= VOLT_LOW_VBUS_VALUE) && (chg->charger_volt <= VOLT_HIGH_VBUS_VALUE)) && (!chg->abnormal_volt_detected)) {
...
static void oplus_chg_smooth_to_soc(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist && chip->batt_exist && (CHARGING_STATUS_FAIL != chip->charging_state) && chip->mmi_chg &&
	    (chip->stop_chg == 1 || chip->charger_type == 5) && status == DISCHARGING_STATUS) {
		status = CHARGING_STATUS;
		time = 0;
		capacity = chip->batt_rm;
		soc_pre = chip->soc;
		smooth_diff = capacity / (2 * chip->soc - chip->smooth_soc);
	} else if (!(chip->charger_exist && chip->batt_exist && chip->mmi_chg && (chip->batt_full || CHARGING_STATUS_FAIL != chip->charging_state) &&
	...
	if (status == DISCHARGING_STATUS) {
		...
		if (chip->smooth_soc > chip->soc) {
			...
			pr_err("[oplus_chg_smooth_to_soc]charging smooth_soc[%d],capacity[%d],smooth_diff[%d],soc_pre[%d]", chip->smooth_soc, capacity,
			       smooth_diff, soc_pre);
		} else {
			chip->smooth_soc = chip->soc;
		}
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
...
#ifndef OPLUS_CHG_OP_DEF
#ifndef WPC_NEW_INTERFACE
	if ((chip->charger_exist || oplus_wireless_charge_start()) && chip->batt_exist && chip->batt_full && chip->mmi_chg &&
	    (chip->stop_chg == 1 || chip->charger_type == 5)) {
#else
...
#endif
#else /* OPLUS_CHG_OP_DEF */
	if ((chip->charger_exist || oplus_chg_is_wls_present(chip)) && chip->batt_exist && chip->batt_full && chip->mmi_chg &&
	    (chip->stop_chg == 1 || chip->charger_type == 5)) {
#endif /* OPLUS_CHG_OP_DEF */
#else
	if (chip->charger_exist && chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
		...
		if (oplus_short_c_batt_is_prohibit_chg(chip)) {
			...
			chip->prop_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
#endif
		} else if ((chip->hmac) && ((chip->tbatt_status == BATTERY_STATUS__NORMAL) || (chip->tbatt_status == BATTERY_STATUS__LITTLE_COOL_TEMP) ||
...
#else
	} else if ((chip->charger_exist || oplus_wpc_get_status()) && chip->batt_exist && (CHARGING_STATUS_FAIL != chip->charging_state) && chip->mmi_chg &&
		   (chip->stop_chg == 1 || chip->charger_type == 5)) {
#endif
#else /* OPLUS_CHG_OP_DEF */
		...
		if (chip->smooth_soc == chip->ui_soc) {
			...
			if (soc_down_count >= soc_down_limit) {
				soc_down_count = 0;
				if (oplus_chg_show_warp_logo_ornot() == false || (chip->warp_show_ui_soc_decimal == false || !chip->decimal_control)) {
					chip->ui_soc--;
				}
			}
		}
		...
		charger_xlog_printk(CHG_LOG_CRTI, "ui_soc:%d,waiting_for_ffc:%d,fastchg_to_ffc:%d,fastchg_start:%d,chg_type=0x%x\n", chip->ui_soc,
				    chip->waiting_for_ffc == false, chip->fastchg_to_ffc == false, oplus_warp_get_fastchg_started(),
				    oplus_warp_get_fast_chg_type());
		if (chip->ui_soc == 100 && chip->fastchg_to_ffc == false &&
		    (oplus_warp_get_fastchg_started() == false || oplus_warp_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_WARP)) {
			if (++cnt >= 12) {
				chip->batt_full = true;
...
static void oplus_chg_get_chargerid_voltage(struct oplus_chg_chip *chip)
{
	if (chip->chg_ops->set_chargerid_switch_val == NULL || chip->chg_ops->get_chargerid_switch_val == NULL || chip->chg_ops->get_chargerid_volt == NULL) {
		return;
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (oplus_warp_get_fastchg_started() == false && reset_mcu_delay < RESET_MCU_DELAY_30S) {
			if (mcu_update == true) {
		...
		if (!oplus_warp_get_fastchg_started() && !oplus_warp_get_fastchg_dummy_started() && !oplus_warp_get_fastchg_to_normal() &&
			...
			} else if (reset_mcu_delay == RESET_MCU_DELAY_30S && chip->vbatt_num == 2) {
				suspend_charger = false;
			...
			if (chip->batt_volt >= chip->limits.ffc1_normal_vfloat_sw_limit) {
				...
				if (vffc1_counts >= FFC_VOLT_COUNTS) {
					...
					charger_xlog_printk(CHG_LOG_CRTI, "batt_volt:%d, ffc1_normal_vfloat_sw_limit:%d, vffc1_counts:%d\n", chip->batt_volt,
							    chip->limits.ffc1_normal_vfloat_sw_limit, vffc1_counts);
					return false;
				}
			}
				...
				if ((chip->icharging * -1) < (chip->limits.ff1_warm_fastchg_ma - chip->limits.ff1_warm_exit_step_ma)) {
					...
					if (warm_counts >= FFC_CURRENT_COUNTS) {
						...
						charger_xlog_printk(CHG_LOG_CRTI,
								    ...
								    chip->icharging, chip->limits.ff1_warm_exit_step_ma, chip->limits.ff1_warm_fastchg_ma,
								    warm_counts);
						return true;
					}
				} else {
				...
				if ((chip->icharging * -1) < (chip->limits.ffc2_normal_fastchg_ma - chip->limits.ffc2_exit_step_ma)) {
					...
					if (normal_counts >= FFC_CURRENT_COUNTS) {
						...
						charger_xlog_printk(CHG_LOG_CRTI,
								    ...
								    chip->icharging, chip->limits.ffc2_normal_fastchg_ma, chip->limits.ffc2_exit_step_ma,
								    normal_counts);
						return true;
					}
				} else {
				...
				if ((chip->icharging * -1) < (chip->limits.ffc2_warm_fastchg_ma - chip->limits.ffc2_warm_exit_step_ma)) {
					...
					if (warm_counts >= FFC_CURRENT_COUNTS) {
						...
						charger_xlog_printk(CHG_LOG_CRTI,
								    ...
								    chip->icharging, chip->limits.ffc2_warm_fastchg_ma, chip->limits.ffc2_warm_exit_step_ma,
								    warm_counts);
						return true;
					}
				} else {
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ctrl_by_warp) {
		if (oplus_warp_get_fastchg_ing() == true && oplus_warp_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_WARP)
			return;
	} else {
		if (oplus_warp_get_fastchg_ing() == true)
			return;
...
#ifdef OPLUS_CHG_OP_DEF
			    (chip->chg_ops->get_charging_enable() == false || oplus_warp_get_ffc_chg_start()) &&
#else
...
#endif
				if (chip->ffc_support && chip->ffc_temp_status != FFC_TEMP_STATUS__HIGH && chip->ffc_temp_status != FFC_TEMP_STATUS__LOW) {
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL) || oplus_chg_check_vbatt_is_full_by_sw(chip)) {
		...
		if ((chip->recharge_after_full == true || chip->vbatt_num == 2) && (chip->tbatt_when_full <= 450 && chip->tbatt_when_full >= 0) &&
	...
	if (chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || chip->boot_mode == LOW_POWER_OFF_CHARGING_BOOT) { /*vbus < 2.5V*/
		if ((chip->chg_ops->check_chrdet_status() == false) && (chip->charger_volt < 2500)) {
			...
			if ((chip->chg_ops->check_chrdet_status() == false) && (chip->chg_ops->get_charger_volt() < 2500)) {
				if ((oplus_warp_get_fastchg_to_normal() == false) && (oplus_warp_get_fastchg_to_warm() == false) &&
				    (oplus_warp_get_adapter_update_status() != ADAPTER_FW_NEED_UPDATE) && (oplus_warp_get_btb_temp_over() == false)) {
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 1) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    ...
				    chip->charger_exist, chip->charger_type, chip->charger_volt, chip->prop_status, chip->boot_mode, chip->batt_exist,
				    chip->batt_full, chip->chging_on, chip->in_rechging, chip->charging_state, chip->total_time, chip->temperature,
				    chip->batt_volt, chip->batt_volt_min, chip->icharging, chip->ibus, chip->soc, chip->ui_soc, chip->soc_load, chip->batt_rm,
				    oplus_gauge_get_batt_fc(), oplus_gauge_get_batt_qm(), oplus_gauge_get_batt_pd(), oplus_gauge_get_batt_rcu(),
				    oplus_gauge_get_batt_rcf(), oplus_gauge_get_batt_fcu(), oplus_gauge_get_batt_fcf(), oplus_gauge_get_batt_sou(),
				    oplus_gauge_get_batt_do0(), oplus_gauge_get_batt_doe(), oplus_gauge_get_batt_trm(), oplus_gauge_get_batt_pc(),
				    oplus_gauge_get_batt_qs(), chip->vbatt_over, chip->chging_over_time, chip->vchg_status, chip->tbatt_status,
				    chip->stop_voter, chip->notify_code, chip->otg_switch, chip->mmi_chg, chip->boot_reason, chip->boot_mode,
		...
		charger_xlog_printk(CHG_LOG_CRTI,
				    ...
				    chip->shell_temp, chip->otg_online, chip->vph_voltage, chip->abnormal_volt_detected, chip->hw_detected);
	}
...
static void oplus_chg_critical_log(struct oplus_chg_chip *chip)
{
	...
	} else if (oplus_warp_get_btb_temp_over() == true || oplus_warp_get_fastchg_to_normal() == true) {
...
static void oplus_chg_ibatt_check_and_set(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ops->need_to_check_ibatt && chip->chg_ops->need_to_check_ibatt() == false) || !chip->chg_ops->need_to_check_ibatt) {
		return;
	}
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->pd_chging == false && chip->chg_ops->oplus_chg_get_pd_type() == true && oplus_chg_show_warp_logo_ornot() == false) {
...
#endif
			chip->limits.temp_little_cool_fastchg_current_ma_high = chip->limits.qc_temp_little_cool_fastchg_current_ma;
			chip->limits.temp_little_cool_fastchg_current_ma_low = chip->limits.qc_temp_little_cool_fastchg_current_ma;
...
static void oplus_chg_update_work(struct work_struct *work)
{
	...
	if (chip->charger_exist && chip->charger_type != POWER_SUPPLY_TYPE_USB && get_eng_version() == HIGH_TEMP_AGING) {
		chg_count++;
	...
	if (!timer_pending(&chip->update_work.timer)) {
		shedule_work = mod_delayed_work(system_highpri_wq, &chip->update_work, OPLUS_CHG_UPDATE_INTERVAL(oplus_chg_update_slow(chip)));
	} else {
...
int oplus_chg_show_warp_logo_ornot(void)
{
...
#ifndef CONFIG_OPLUS_CHARGER_MTK
#ifndef OPLUS_CHG_OP_DEF
	if (g_charger_chip->wireless_support && oplus_wpc_get_adapter_type() == CHARGER_SUBTYPE_FASTCHG_SWARP &&
...
#else /* OPLUS_CHG_OP_DEF */
	if ((oplus_chg_get_wls_charge_type(g_charger_chip) == OPLUS_CHG_WLS_WARP || oplus_chg_get_wls_charge_type(g_charger_chip) == OPLUS_CHG_WLS_SWARP ||
	...
	if (oplus_warp_get_fastchg_started()) {
		return 1;
	} else if (oplus_warp_get_fastchg_to_normal() == true || oplus_warp_get_fastchg_to_warm() == true || oplus_warp_get_fastchg_dummy_started() == true ||
...
struct current_level SWARP_2_0[] = {
	{ 1, 1000 },  { 2, 1500 },   { 3, 2000 },   { 4, 2500 },   { 5, 3000 },	  { 6, 3500 },	 { 7, 4000 },	{ 8, 4500 },  { 9, 5000 },
	{ 10, 5500 }, { 11, 6000 },  { 12, 6300 },  { 13, 6500 },  { 14, 7000 },  { 15, 7500 },	 { 16, 8000 },	{ 17, 8500 }, { 18, 9000 },
	{ 19, 9500 }, { 20, 10000 }, { 21, 10500 }, { 22, 11000 }, { 23, 11500 }, { 24, 12000 }, { 25, 12500 },
};

struct current_level SWARP_1_0[] = { { 1, 2000 }, { 2, 3000 }, { 3, 4000 },  { 4, 5000 },  { 5, 6000 },	  { 6, 7000 },
				     { 7, 8000 }, { 8, 9000 }, { 9, 10000 }, { 10, 1200 }, { 11, 12000 }, { 12, 12600 } };
...
int oplus_chg_override_by_shell_temp(int temp)
{
	...
	if ((g_charger_chip->charger_exist) && g_charger_chip->smart_charging_screenoff && (temp > BATT_NTC_CTRL_THRESHOLD_LOW) &&
...
#ifdef CONFIG_OPLUS_CHARGER_MTK
	if (boot_mode == META_BOOT || boot_mode == FACTORY_BOOT || boot_mode == ADVMETA_BOOT || boot_mode == ATE_FACTORY_BOOT) {
...
static int oplus_tbatt_power_off_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (get_eng_version() != HIGH_TEMP_AGING) {
			if (over_temp_count >= OPLUS_TBATT_HIGH_PWROFF_COUNT || emergency_count >= OPLUS_TBATT_EMERGENCY_PWROFF_COUNT) {
