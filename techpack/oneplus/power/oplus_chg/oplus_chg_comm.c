...
static void oplus_chg_set_charge_done(struct oplus_chg_comm *comm_dev, bool done)
{
	...
	oplus_chg_global_event(comm_dev->comm_ocm, done ? OPLUS_CHG_EVENT_CHARGE_DONE : OPLUS_CHG_EVENT_CLEAN_CHARGE_DONE);
}
...
static ssize_t oplus_chg_comm_charge_parameter_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;
...
static ssize_t oplus_chg_comm_charge_parameter_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static enum oplus_chg_mod_property oplus_chg_comm_props[] = {
	OPLUS_CHG_PROP_CHG_ENABLE, OPLUS_CHG_PROP_SHIP_MODE, OPLUS_CHG_PROP_SKIN_TEMP, OPLUS_CHG_PROP_CALL_ON, OPLUS_CHG_PROP_CAMERA_ON,
};
...
static int oplus_chg_comm_get_prop(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop, union oplus_chg_mod_propval *pval)
{
	struct oplus_chg_comm *comm_dev = oplus_chg_mod_get_drvdata(ocm);
...
static int oplus_chg_comm_set_prop(struct oplus_chg_mod *ocm, enum oplus_chg_mod_property prop, const union oplus_chg_mod_propval *pval)
{
	struct oplus_chg_comm *comm_dev = oplus_chg_mod_get_drvdata(ocm);
	struct oplus_chg_chip *chip = oplus_chg_get_chg_struct();
	int rc = 0;
...
static int oplus_chg_comm_event_notifier_call(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	case OPLUS_CHG_EVENT_ONLINE:
		...
		if (!strcmp(owner_ocm->desc->name, "wireless")) {
			...
			schedule_delayed_work(&comm_dev->heartbeat_work, round_jiffies_relative(msecs_to_jiffies(HEARTBEAT_INTERVAL_MS)));
...
static int oplus_chg_comm_parse_dt(struct oplus_chg_comm *comm_dev)
{
	...
	pr_info("wkcs: temp thr = %d, %d, %d, %d, %d, %d, %d\n", comm_cfg->batt_temp_thr[0], comm_cfg->batt_temp_thr[1], comm_cfg->batt_temp_thr[2],
		comm_cfg->batt_temp_thr[3], comm_cfg->batt_temp_thr[4], comm_cfg->batt_temp_thr[5], comm_cfg->batt_temp_thr[6]);
	...
	rc = read_signed_data_from_node(node, "oplus,ffc-temp-thr", (s32 *)comm_cfg->ffc_temp_thr, FFC_TEMP_INVALID - 1);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,usb-ffc-fv-mv", (u32 *)comm_cfg->usb_ffc_fv_mv, comm_cfg->usb_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,usb-ffc-fv-cutoff-mv", (u32 *)comm_cfg->usb_ffc_fv_cutoff_mv, comm_cfg->usb_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,usb-ffc-fcc-ma", (u32 *)comm_cfg->usb_ffc_fcc_ma, (FFC_TEMP_INVALID - 2) * comm_cfg->usb_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,wls-ffc-fv-mv", (u32 *)comm_cfg->wls_ffc_fv_mv, comm_cfg->wls_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,wls-ffc-fv-cutoff-mv", (u32 *)comm_cfg->wls_ffc_fv_cutoff_mv, comm_cfg->wls_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,wls-ffc-icl-ma", (u32 *)comm_cfg->wls_ffc_icl_ma, (FFC_TEMP_INVALID - 2) * comm_cfg->wls_ffc_step_max);
	if (rc < 0) {
	...
	rc = read_unsigned_data_from_node(node, "oplus,wls-ffc-fcc-ma", (u32 *)comm_cfg->wls_ffc_fcc_ma, (FFC_TEMP_INVALID - 2) * comm_cfg->wls_ffc_step_max);
	if (rc < 0) {
...
static int oplus_chg_comm_check_batt_temp(struct oplus_chg_comm *comm_dev)
{
	...
	*/
	...
	if (comm_dev->temp_region > temp_region) {
		for (i = 0; i < BATT_TEMP_INVALID - 1; i++) {
			if (i == temp_region)
				comm_dev->batt_temp_dynamic_thr[i] = comm_dev->batt_temp_dynamic_thr[i] + BATT_TEMP_HYST;
			else
				comm_dev->batt_temp_dynamic_thr[i] = comm_cfg->batt_temp_thr[i];
		}
		comm_dev->is_power_changed = true;
...
int oplus_chg_comm_switch_ffc(struct oplus_chg_mod *comm_ocm)
{
	...
	if (ffc_temp_region == FFC_TEMP_PRE_NORMAL || ffc_temp_region == FFC_TEMP_NORMAL) {
		if (is_usb_charger_online(comm_dev)) {
			...
			vote(comm_dev->wls_icl_votable, USER_VOTER, true, comm_cfg->wls_ffc_icl_ma[comm_dev->ffc_step][ffc_temp_region - 1], true);
			vote(comm_dev->wls_fv_votable, USER_VOTER, true, comm_cfg->wls_ffc_fv_mv[comm_dev->ffc_step], false);
			vote(comm_dev->wls_fcc_votable, USER_VOTER, true, comm_cfg->wls_ffc_fcc_ma[comm_dev->ffc_step][ffc_temp_region - 1], false);
		} else {
		...
		if (pre_ffc_temp_region != ffc_temp_region) {
			...
			if (ffc_temp_region == FFC_TEMP_PRE_NORMAL || ffc_temp_region == FFC_TEMP_NORMAL) {
				if (usb_online) {
					...
					vote(comm_dev->wls_icl_votable, USER_VOTER, true, comm_cfg->wls_ffc_icl_ma[comm_dev->ffc_step][ffc_temp_region - 1],
					     true);
					vote(comm_dev->wls_fcc_votable, USER_VOTER, true, comm_cfg->wls_ffc_fcc_ma[comm_dev->ffc_step][ffc_temp_region - 1],
					     false);
				} else {
					pr_err("Unknown charger type\n");
					rc = -EINVAL;
...
static void oplus_chg_heartbeat_work(struct work_struct *work)
{
	...
	if (temp_region <= BATT_TEMP_COLD || temp_region >= BATT_TEMP_HOT)
		vbatdet_mv = 0;
	if (!comm_dev->chg_ovp && comm_dev->chg_done && !comm_dev->recharge_pending && temp_region > BATT_TEMP_COLD && temp_region < BATT_TEMP_HOT &&
	    vbatdet_mv >= vbat_mv) {
		oplus_chg_set_charge_done(comm_dev, false);
...
static int oplus_chg_comm_get_iio_channel(struct oplus_chg_comm *comm_dev, const char *propname, struct iio_channel **chan)
{
	struct device_node *node = comm_dev->dev->of_node;
	int rc;
